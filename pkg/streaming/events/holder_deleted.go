...
// Holder deletion-type values carried on holder.deleted. "soft" sets DeletedAt
// and retains the record; "hard" purges it. Consumers branch on this to decide
// retention/erasure semantics.
const (
	holderDeletionTypeSoft = "soft"
	holderDeletionTypeHard = "hard"
)
...
func NewHolderDeleted(id, organizationID string, hardDelete bool, deletedAt time.Time) HolderDeletedPayload {
	deletionType := holderDeletionTypeSoft
	if hardDelete {
		deletionType = holderDeletionTypeHard
	}
