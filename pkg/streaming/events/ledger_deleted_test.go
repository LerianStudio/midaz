...
func TestLedgerDeletedPayload_ToEvent_AssemblesStreamingEvent(t *testing.T) {
	...
	assert.Equal(t, events.LedgerDeletedDefinition.ResourceType, evt.ResourceType)
	assert.Equal(t, events.LedgerDeletedDefinition.EventType, evt.EventType)
	assert.Equal(t, events.LedgerDeletedDefinition.SchemaVersion, evt.SchemaVersion)
