...
func TestOperationRouteDeletedDefinition_Key(t *testing.T) {
	...
	assert.Equal(t, "operation-route.deleted", events.OperationRouteDeletedDefinition.RouteKey())
...
func TestNewOperationRouteDeleted_MapsIdentity(t *testing.T) {
	payload := events.NewOperationRouteDeleted(
		operationRouteID.String(),
		operationRouteOrg.String(),
		operationRouteLed.String(),
		fixedTime,
	)
...
func TestOperationRouteDeletedPayload_ToEmitRequest_AssemblesStreamingEvent(t *testing.T) {
	payload := events.NewOperationRouteDeleted("or-123", "org-456", "led-789", fixedTime)

	req, err := payload.ToEmitRequest("tenant-1", fixedTime)
	require.NoError(t, err)

	assert.Equal(t, events.OperationRouteDeletedDefinition.Key(), req.DefinitionKey)
	assert.Equal(t, "tenant-1", req.TenantID)
	assert.Equal(t, payload.ID, req.Subject)
	assert.Equal(t, fixedTime, req.Timestamp)

	var roundTrip events.OperationRouteDeletedPayload
	require.NoError(t, json.Unmarshal(req.Payload, &roundTrip))
	assert.Equal(t, payload, roundTrip)
}

func TestOperationRouteDeletedPayload_JSONShape(t *testing.T) {
	payload := events.NewOperationRouteDeleted("or-123", "org-456", "led-789", fixedTime)
