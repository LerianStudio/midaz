...
// `transaction_route` — it is the Key() and the ce-type. Definition.RouteKey()
// folds it back to `transaction-route` for the RouteDefinition.Key only,
// because the lib-streaming route-key regex rejects underscores.
