...
// TestInstrumentUpdatedDefinition_Key locks the canonical event key.
func TestInstrumentUpdatedDefinition_Key(t *testing.T) {
	assert.Equal(t, "instrument.updated", events.InstrumentUpdatedDefinition.Key())
	assert.Equal(t, "instrument", events.InstrumentUpdatedDefinition.ResourceType)
	assert.Equal(t, "updated", events.InstrumentUpdatedDefinition.EventType)
	assert.Equal(t, "1.0.0", events.InstrumentUpdatedDefinition.SchemaVersion)
}

// TestNewInstrumentUpdated_MapsMinimalInstrument verifies the happy-path mapping
// for the simplest instrument.
func TestNewInstrumentUpdated_MapsMinimalInstrument(t *testing.T) {
	i := minimalInstrument()

	payload := events.NewInstrumentUpdated(i, instrumentTestOrgID)
...
// TestNewInstrumentUpdated_MapsRelatedParties covers the multi-entry path and
// PROVES only relatedPartyId + role cross the wire.
func TestNewInstrumentUpdated_MapsRelatedParties(t *testing.T) {
...
// TestInstrumentUpdatedPayload_ToEmitRequest_AssemblesStreamingEvent verifies
// the ToEmitRequest helper composes a fully-populated EmitRequest.
func TestInstrumentUpdatedPayload_ToEmitRequest_AssemblesStreamingEvent(t *testing.T) {
	payload := events.NewInstrumentUpdated(instrumentWithRelatedParties(), instrumentTestOrgID)

	req, err := payload.ToEmitRequest("tenant-1", instrumentFixedTime)
	require.NoError(t, err)

	assert.Equal(t, events.InstrumentUpdatedDefinition.Key(), req.DefinitionKey)
	assert.Equal(t, "tenant-1", req.TenantID)
	assert.Equal(t, payload.ID, req.Subject)
	assert.Equal(t, instrumentFixedTime, req.Timestamp)

	var roundTrip events.InstrumentUpdatedPayload
	require.NoError(t, json.Unmarshal(req.Payload, &roundTrip))
	assert.Equal(t, payload, roundTrip)
}

// TestInstrumentUpdatedPayload_JSONShape_NilRelatedPartiesIsNull locks the
// intended wire contract for an instrument with no related parties:
// relatedParties MUST encode as JSON null, never as an empty array. A future
// switch to a non-nil empty slice would silently flip the wire to [] and break
// consumers that distinguish absent from empty.
func TestInstrumentUpdatedPayload_JSONShape_NilRelatedPartiesIsNull(t *testing.T) {
	payload := events.NewInstrumentUpdated(minimalInstrument(), instrumentTestOrgID)
...
// TestInstrumentUpdatedPayload_JSONShape locks the wire JSON layout and asserts
// PII absence at the top level and inside each relatedParties element.
func TestInstrumentUpdatedPayload_JSONShape(t *testing.T) {
	payload := events.NewInstrumentUpdated(instrumentWithRelatedParties(), instrumentTestOrgID)
