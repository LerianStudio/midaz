...
func TestBalanceRedis_UnmarshalJSON(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantOverdraftUsed: "9007199254740995",
			wantVersion:       9007199254740993,
			wantErr:           false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantOverdraftUsed != "" {
				assert.Equal(t, tt.wantOverdraftUsed, br.OverdraftUsed)
			}
			if tt.wantVersion != 0 {
				assert.Equal(t, tt.wantVersion, br.Version)
			}
		})
	}
}
