...
// TestTransaction_EffectiveRouteID pins the precedence between the canonical
// route identifier and the deprecated route string, the value every fee package
// is scoped against. A payment created through the current path carries only the
// canonical identifier; a payment created through the legacy contract carries
// only the deprecated string; and a canonical identifier that arrived empty is
// not a route, so the legacy string still answers.
func TestTransaction_EffectiveRouteID(t *testing.T) {
	t.Parallel()

	routeID := "5a0b7f6e-0f1a-4a55-9c4c-6c1c1a7f3b21"
	legacyRoute := "9f2c4d10-1b3e-4c88-a1f2-77b0d3e5c904"
	blank := ""
	...
	tests := []struct {
		...
		{
			name:        "canonical route identifier answers",
			transaction: Transaction{RouteID: &routeID},
			want:        routeID,
		},
		{
			name:        "canonical identifier wins over the deprecated string",
			transaction: Transaction{RouteID: &routeID, Route: legacyRoute},
			want:        routeID,
		},
		{
			name:        "legacy payment falls back to the deprecated string",
			transaction: Transaction{Route: legacyRoute},
			want:        legacyRoute,
		},
		{
			name:        "an empty canonical identifier is not a route, so the deprecated string answers",
			transaction: Transaction{RouteID: &blank, Route: legacyRoute},
			want:        legacyRoute,
		},
		{
			name:        "a payment carrying no route at all scopes against nothing",
			transaction: Transaction{},
			want:        "",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.want, tt.transaction.EffectiveRouteID())
		})
	}
}
