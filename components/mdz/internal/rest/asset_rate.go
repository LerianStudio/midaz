...
func (r *assetRate) Create(
	...
	var assetRateRest mmodel.AssetRate
	if err := json.NewDecoder(resp.Body).Decode(&assetRateRest); err != nil {
		return nil, errors.New("decoding response JSON:" + err.Error())
	}

	return &assetRateRest, nil
}

func (r *assetRate) Update(
	organizationID, ledgerID, assetRateID string,
	inp mmodel.UpdateAssetRateInput,
) (*mmodel.AssetRate, error) {
	jsonData, err := json.Marshal(inp)
	if err != nil {
		return nil, fmt.Errorf("marshalling JSON: %v", err)
...
func (r *assetRate) GetByID(
	organizationID, ledgerID, assetRateID string) (*mmodel.AssetRate, error) {
	uri := fmt.Sprintf("%s/v1/organizations/%s/ledgers/%s/asset-rates/%s",
		r.Factory.Env.URLAPITransaction, organizationID, ledgerID, assetRateID)
...
func (r *assetRate) GetByAssetCode(
	...
	baseURL := fmt.Sprintf("%s/v1/organizations/%s/ledgers/%s/asset-rates/from/%s",
		r.Factory.Env.URLAPITransaction, organizationID, ledgerID, assetCode)
