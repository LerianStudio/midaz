...
func newCmdAccountList(f *factoryAccountList) *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: utils.Format(
			"Returns a list of all accounts within the specified ledger.",
		),
		Example: utils.Format(
			"$ mdz account list",
			"$ mdz account list -h",
			"$ mdz account list --limit 10",
			"$ mdz account list --organization-id 43212 --ledger-id 12333",
			"$ mdz account list --organization-id 43212 --ledger-id 12333 --page 3",
			"$ mdz account list --organization-id 43212 --ledger-id 12333 --limit 10 --page 2",
		),
		RunE: f.runE,
	}
