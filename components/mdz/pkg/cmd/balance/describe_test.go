...
func Test_newCmdBalanceDescribe(t *testing.T) {
	t.Run("happy path informing all the necessary flags", func(t *testing.T) {
		...
		balanceFactory := factoryBalanceDescribe{
		...
		cmd := newCmdBalanceDescribe(&balanceFactory)
		cmd.SetArgs([]string{
		...
		mockRepo.EXPECT().GetByID(organizationID, ledgerID, balanceID).Return(result, nil)
		err := cmd.Execute()
		assert.NoError(t, err)

		output := balanceFactory.factory.IOStreams.Out.(*bytes.Buffer).String()
		assert.Contains(t, output, balanceID)
