...
// This file contains the list_test command implementation.
...
func Test_newCmdSegmentList(t *testing.T) {
	t.Run("with flags", func(t *testing.T) {
		...
		cmd := newCmdSegmentList(&ledFactory)
		cmd.SetArgs([]string{
			"--organization-id", organizationID,
			"--ledger-id", ledgerID,
		})

		list := &mmodel.Segments{
			Page:  1,
			Limit: 2,
