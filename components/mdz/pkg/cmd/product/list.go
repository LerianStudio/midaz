...
func newCmdProductList(f *factoryProductList) *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: utils.Format(
			"$ mdz cluster list",
			"$ mdz cluster list -h",
			"$ mdz cluster list --limit 10",
			"$ mdz cluster list --organization-id 43212 --ledger-id 12333",
			"$ mdz cluster list --organization-id 43212 --ledger-id 12333 --page 3",
			"$ mdz cluster list --organization-id 43212 --ledger-id 12333 --limit 10 --page 2",
		),
		RunE: f.runE,
	}
