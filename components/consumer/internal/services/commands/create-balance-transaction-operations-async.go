...
func (uc *UseCase) CreateBalanceTransactionOperationsAsync(ctx context.Context, data mmodel.Queue) error {
	...
	ctxProcessBalances, spanUpdateBalances := tracer.Start(ctx, "commands.create_balance_transaction_operations.update_balances")
	...
	ctxProcessMetadata, spanCreateMetadata := tracer.Start(ctx, "commands.create_balance_transaction_operations.create_metadata")
	...
	ctxProcessOperation, spanCreateOperation := tracer.Start(ctx, "commands.create_balance_transaction_operations.create_operation")
...
func (uc *UseCase) CreateOrUpdateTransaction(ctx context.Context, logger libLog.Logger, tracer trace.Tracer, t transaction.TransactionQueue) (*transaction.Transaction, error) {
	_, spanCreateTransaction := tracer.Start(ctx, "commands.create_balance_transaction_operations.create_transaction")
