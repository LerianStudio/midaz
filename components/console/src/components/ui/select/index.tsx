...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      'flex h-10 w-full items-center justify-between gap-3 rounded-md border border-[#C7C7C7] bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-shadcn-400 focus:outline-hidden focus:ring-2 focus:ring-ring focus:ring-offset-0 focus-visible:outline-hidden [&>span]:line-clamp-1',
      props.readOnly && [
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ref={ref}
        className={cn(
          'group/select relative z-50 max-h-96 min-w-32 overflow-hidden rounded-md border bg-popover text-popover-foreground shadow-md data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2',
          position === 'popper' &&
...
>(({ className, ...props }, ref) => (
  <div
    ref={ref}
    className={cn(
      'hidden py-6 text-center text-sm text-muted-foreground group-data-[empty=true]/select:block',
      className
    )}
    {...props}
  />
