...
/**
 * Setup test ledger in the database
 * Runs the insert-ledger.sql script against the test database
 * Uses ORGANIZATION_ID from environment variable
 */
export async function setupLedger() {
  const sqlFilePath = join(__dirname, 'insert-ledger.sql')
  ...
  try {
    ...
    console.log('✓ Test ledger inserted successfully')
    return true
  } catch (error) {
    console.error('Failed to setup ledger:', error)
    throw error
  }
}
