...
// Package crm reproduces the crm-specific report extraction behavior that the
// generic embedded engine path does not provide: the hash-based advanced-filter
// rewrite applied before extraction, the organization-scoped collection fan-out
// applied during extraction, and the field decryption applied after extraction.
//
// It exists as a self-contained stage so the generate-report handler can wire
// two seams around the engine:
//
//	(1) PRE-FILTER  — TransformFilters rewrites crm field-name filters to
...
//	    decryption applied to crm result rows.
//
// The crypto keys are passed in by the caller
// (UseCase.CryptoHashSecretKeyCRM / UseCase.CryptoEncryptSecretKeyCRM); this
// package neither holds nor relocates secrets. It never logs decrypted values,
// secrets, hashes, or PII.
package crm
...
const (
	// DatasourceName is the logical datasource name that selects the crm
	// extraction path. The stage applies when databaseName == "crm" and no other
...
// advancedFilterFieldMappings maps a crm encrypted/logical field name to its
// hashed search.* counterpart. A filter on an encrypted field cannot match
...
// values. This is the wire contract with the plugin's stored search index.
...
// Is reports whether a datasource name selects the crm extraction path. It is
// the single predicate the handler uses to decide whether to apply this stage
// (databaseName == "crm").
func Is(datasourceName string) bool {
	return datasourceName == DatasourceName
}

// IsQueryableCollection reports whether a crm collection should be extracted.
// The "organization" collection is metadata (it carries the organization_id for
// template context), not a physical collection, so it is skipped.
func IsQueryableCollection(collection string) bool {
	return collection != OrganizationCollection
}

// TransformFilters rewrites a single collection's advanced filters for the crm
// search index. Mapped field names are replaced with their hashed search.*
// counterparts and their string values are HMAC-hashed with the configured hash
// key; unmapped fields are passed through unchanged.
//
...
// produces an insecure, non-matching hash, so a filtered crm extraction must not
// proceed.
//
// The hashKey is supplied by the caller (UseCase.CryptoHashSecretKeyCRM); this
// function neither holds nor logs it.
...
// equivalents. An operator left empty in the source stays empty in the result.
...
// slice, leaving non-string and empty-string values untouched.
