...
// hub and topic provisioning are pointed at the stream tracer actually writes.
//
...
func TestBuildStreamingManifestHandler_TopicFollowsConfiguredSource(t *testing.T) {
	...
	const configuredSource = "midaz-tracer"

	doc := fetchTracerManifest(t, &Config{StreamingCloudEventsSource: configuredSource})
	...
	require.Equal(t, "lerian.streaming.midaz-tracer", doc.Topic)
}
