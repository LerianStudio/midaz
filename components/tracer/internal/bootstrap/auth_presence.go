...
// AuthGuard.Protect falls through to optional APIKeyAuth, which calls c.Next()
// unconditionally when disabled — ordinary /v1 routes serve unauthenticated.
// Financial reservation routes do not use Protect: their always-on API-key
// guard remains fail-closed in every deployment mode.
//
...
func ValidateAuthPresence(ctx context.Context, cfg *Config, logger libLog.Logger) error {
	...
	if strings.EqualFold(strings.TrimSpace(mode), "local") {
		...
		).Log(ctx, libLog.LevelWarn, "ALL optional authentication is DISABLED — ordinary /v1 routes are open; financial reservation routes still require X-API-Key (acceptable for local development only)")

		return nil
	}

	return fmt.Errorf(
		"DEPLOYMENT_MODE=%q requires at least one auth mechanism: set API_KEY_ENABLED=true or PLUGIN_AUTH_ENABLED=true; running non-local without authentication leaves ordinary /v1 routes open (financial reservation routes remain API-key protected)",
		mode,
	)
}
