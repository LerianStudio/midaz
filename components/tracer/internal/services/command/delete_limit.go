...
func (c *DeleteLimitCommand) Execute(ctx context.Context, id uuid.UUID) (retErr error) {
	...
	start := time.Now()
	defer func() {
		utils.RecordDomainOperation(ctx, factory, logger, "tracer", "limit_delete", start, retErr)
	}()
