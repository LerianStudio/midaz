...
func (c *DeactivateLimitCommand) Execute(ctx context.Context, id uuid.UUID) (_ *model.Limit, retErr error) {
	...
	if limit.Status == model.LimitStatusInactive {
		logger.With(
			libLog.String("operation", "service.limit.deactivate"),
			libLog.String("limit.id", id.String()),
		).Log(ctx, libLog.LevelDebug, "Limit already inactive (idempotent no-op)")
