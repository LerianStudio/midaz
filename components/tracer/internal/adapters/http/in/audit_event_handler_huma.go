...
type ListAuditEventsInputHuma struct {
	...
	EventType       string `query:"event_type" doc:"Filter by event type (TRANSACTION_VALIDATED, RULE_*, LIMIT_*, POLICY_PUBLISHED, POLICY_BOUND)"`
	...
	ResourceType    string `query:"resource_type" doc:"Filter by resource type (transaction, rule, limit, policy)"`
