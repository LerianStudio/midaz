...
func TestContextReserveNativeGRPC(t *testing.T) {
	for _, scenario := range []string{"registered producer", "unknown producer", "legacy wire", "missing presence", "unsupported revision"} {
		t.Run(scenario, func(t *testing.T) {
			...
			case "registered producer":
				...
				completion.EXPECT().ExecuteReport(gomock.Any(), request.TransactionID, model.OperationConfirmed).Return(&tracercontract.TransactionCompletionResult{ContractRevision: request.ContractRevision, TransactionID: request.TransactionID, Status: "CONFIRMED"}, nil)
			...
			server := grpc.NewServer(grpc.Creds(credentials.NewTLS(serverTLS)), grpc.ChainUnaryInterceptor(grpcin.IdentityUnaryInterceptor(resolver)), grpc.MaxRecvMsgSize(65536))
			...
			defer cancel()
			ctx = metadata.NewOutgoingContext(ctx, metadata.Pairs("x-integration-id", "forged", "x-asset-namespace", "forged"))
