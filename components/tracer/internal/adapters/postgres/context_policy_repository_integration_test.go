...
func TestIntegrationContextPolicyMigrationPreservesUsage(t *testing.T) {
	...
	dir := t.TempDir()
	require.NoError(t, migrations.WriteTo(dir))
	// Roll back the newer decision FK before its policy target, then restore
	// them in forward order. No stored decisions exist in this shared fixture.
	for _, file := range []string{"000028_reserve_decisions.down.sql", "000025_context_policies.down.sql", "000025_context_policies.up.sql", "000028_reserve_decisions.up.sql"} {
