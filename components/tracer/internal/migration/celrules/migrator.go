...
// table, rewrites and RECOMPILES every rule against the new environment, and
// commits only if all recompile. Any failure rolls the whole transaction back,
// so no partial rewrite is ever persisted.
//
...
// NewMigrator wires the migration collaborators. All are required.
func NewMigrator(txBeginner pgdb.TxBeginner, store RuleStore, compiler ExpressionCompiler, rewrite Rewriter) (*Migrator, error) {
	if txBeginner == nil {
...
// Up rewrites every stored rule's currency reference to asset in one atomic
// transaction: lock, rewrite-all, recompile-all gate, persist-all, commit. Any
// error rolls back the whole transaction, so a failed recompile leaves the rule
// table untouched.
func (m *Migrator) Up(ctx context.Context) (Result, error) {
	...
	for _, rule := range rules {
		...
		if err != nil {
			libOtel.HandleSpanError(span, "Failed to rewrite rule expression", err)
			return res, fmt.Errorf("rewrite rule %s: %w", rule.ID, err)
		}

		if err := m.compiler.Compile(rewritten); err != nil {
			libOtel.HandleSpanError(span, "Rewritten expression failed the recompile-all gate", err)
