...
// BadRequestWithMessage sends an HTTP 400 Bad Request response with a structured error format.
// This is the preferred method for validation errors as it follows the API specification.
func BadRequestWithMessage(c *fiber.Ctx, code, title, message string) error {
...
// GatewayTimeout sends an HTTP 504 Gateway Timeout response with a custom code, title and message.
