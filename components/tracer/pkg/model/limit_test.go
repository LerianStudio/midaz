...
func TestNewLimit(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "rejects invalid asset length",
			limitName:   "Test Limit",
			limitType:   LimitTypeDaily,
			maxAmount:   decimal.RequireFromString("1000"),
			asset:       strings.Repeat("x", 257),
			scopes:      []Scope{validScope},
			expectError: true,
			errorIs:     constant.ErrLimitInvalidCurrency,
		},
		...
		{
			name:          "preserves mixed case asset",
			limitName:     "Test Limit",
			limitType:     LimitTypeDaily,
			maxAmount:     decimal.RequireFromString("1000"),
			asset:         "wBTC",
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			limit, err := NewLimit(tc.limitName, tc.limitType, tc.maxAmount, tc.asset, tc.scopes, tc.description, testutil.FixedTime())
