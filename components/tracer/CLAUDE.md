...
| **Language** | Go (root `go.mod`: `go 1.26.4`; Dockerfile builder: `golang:1.26.3-alpine`) |
...
make coverage-summary # Quick coverage verification
...
make generate-docs    # Generate Swagger/OpenAPI docs (run from repo root; covers ledger, tracer, reporter)
```
...
│   ├── constant/                  # Tracer-local pagination/app constants (error codes live in the repo-root canonical registry)
...
│   ├── hash/                      # FNV-1a UUID→int32 (advisory-lock keys); audit SHA-256 chain is DB-side
...
Binding standard: [`../../docs/standards/error-handling.md`](../../docs/standards/error-handling.md) (E1–E14) and [`../../docs/standards/telemetry.md`](../../docs/standards/telemetry.md) (T5/T8). Tracer uses the ONE platform — typed errors and sentinels live in the repo-root `pkg/errors.go` + `pkg/constant/errors.go`; there is no tracer-local error fork, and the `TRC-` prefixed code family is retired in favor of the canonical numeric registry.

- Sentinel errors referenced by constant from the canonical registry (`pkg/constant/errors.go`)
- Record onto the span by error CLASS (T5): business/4xx -> `HandleSpanBusinessErrorEvent`, technical/5xx -> `HandleSpanError`
- Business errors: return directly. Technical errors: wrap with `fmt.Errorf("context: %w", err)`
...
Binding standard: [`../../docs/standards/telemetry.md`](../../docs/standards/telemetry.md) (T6 structured logging, T7 log levels, T8 single-point logging).

- Constant message + typed fields, never string interpolation (T6)
- Enrich with trace context: `logger = logging.WithTrace(ctx, logger)`
- `operation` field must match span name
- Log each error ONCE, at the boundary that owns the handling decision (HTTP handler / consumer loop) — inner layers record onto the span and return without logging (T8)
- Info is sanctioned milestones only (T7): do NOT emit per-request narration such as "Creating rule" / "Rule created successfully" / "Retrieving ..."
...
- **lib-commons/v6** (`v6.1.0`): Common utilities, infrastructure
...
| [`../../pkg/constant/errors.go`](../../pkg/constant/errors.go) | Canonical error-code registry (repo-root, shared platform — no tracer-local fork) |
| [`pkg/model/`](pkg/model/) | Domain entities |
