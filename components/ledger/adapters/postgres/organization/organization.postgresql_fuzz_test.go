...
//   2. Path selection invariants are preserved: "onboarding" -> tenant DB,
...
//	    ./components/ledger/adapters/postgres/organization/
//
// =============================================================================
...
//   - No panic
//   - moduleName == "onboarding" -> returns the tenant DB (no error)
//   - moduleName != "onboarding" -> takes the fallback path (static connection)
...
func FuzzGetDB_ContextModuleName(f *testing.F) {
	...
	f.Add("onboarding'; DROP TABLE organization;--")
	// 6. Boundary: very long string (256 chars)
	f.Add(strings.Repeat("x", 256))
	// 7. Security: null byte injection
	f.Add("onboarding\x00extra")
	// 8. Boundary: case variation (should NOT match "onboarding")
	f.Add("ONBOARDING")
	...
	f.Fuzz(func(t *testing.T, moduleName string) {
		...
		// Invariant 1: when moduleName is exactly "onboarding", getDB must
		// return the tenant DB from context without error.
		if moduleName == "onboarding" {
			require.NoError(t, err,
				"getDB must not error when context has 'onboarding' module DB")
			require.NotNil(t, db,
				"getDB must return a non-nil DB for 'onboarding' module")
			assert.Same(t, tenantDB, db,
				"getDB must return the exact tenant DB injected into context")
		}

		// Invariant 2: when moduleName is NOT "onboarding", getDB falls back
