...
func TestMigration000025_FilesExist(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			filename: "000025_add_route_description_to_operation.up.sql",
		},
		{
			name:     "down migration file exists",
			filename: "000025_add_route_description_to_operation.down.sql",
		},
	}
...
func TestMigration000025_UpSQL_AddRouteDescriptionColumn(t *testing.T) {
	t.Parallel()

	dir := migrationsDir(t)
	path := filepath.Join(dir, "000025_add_route_description_to_operation.up.sql")
...
func TestMigration000025_DownSQL_DropRouteDescriptionColumn(t *testing.T) {
	t.Parallel()

	dir := migrationsDir(t)
	path := filepath.Join(dir, "000025_add_route_description_to_operation.down.sql")
