...
func TestMigration000040_AddsGroupLookupIndexWithoutLedgerScope(t *testing.T) {
	...
	assert.Contains(t, upSQL, "create index concurrently if not exists idx_transaction_group")
	...
	assert.Contains(t, downSQL, "drop index concurrently if exists idx_transaction_group")
}
