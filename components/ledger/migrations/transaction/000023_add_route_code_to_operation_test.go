...
func TestMigration000023_FilesExist(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			filename: "000023_add_route_code_to_operation.up.sql",
		},
		{
			name:     "down migration file exists",
			filename: "000023_add_route_code_to_operation.down.sql",
		},
	}
...
func TestMigration000023_UpSQL_AddRouteCodeColumn(t *testing.T) {
	t.Parallel()

	dir := migrationsDir(t)
	path := filepath.Join(dir, "000023_add_route_code_to_operation.up.sql")
...
func TestMigration000023_DownSQL_DropRouteCodeColumn(t *testing.T) {
	t.Parallel()

	dir := migrationsDir(t)
	path := filepath.Join(dir, "000023_add_route_code_to_operation.down.sql")
