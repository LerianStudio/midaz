...
func TestMigration000022_FilesExist(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			filename: "000022_add_accounting_entries_to_operation_route.up.sql",
		},
		{
			name:     "down migration file exists",
			filename: "000022_add_accounting_entries_to_operation_route.down.sql",
		},
	}
...
func TestMigration000022_UpSQL_AddAccountingEntriesColumn(t *testing.T) {
	t.Parallel()

	dir := migrationsDir(t)
	path := filepath.Join(dir, "000022_add_accounting_entries_to_operation_route.up.sql")
...
func TestMigration000022_DownSQL_DropAccountingEntriesColumn(t *testing.T) {
	t.Parallel()

	dir := migrationsDir(t)
	path := filepath.Join(dir, "000022_add_accounting_entries_to_operation_route.down.sql")
...
func TestMigration000022_UpSQL_IdempotentRerun(t *testing.T) {
	t.Parallel()
...
func TestMigration000022_UpSQL_ExistingRowsUnaffected(t *testing.T) {
	t.Parallel()
