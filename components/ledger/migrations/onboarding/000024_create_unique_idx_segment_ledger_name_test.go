...
// TestMigration000024_FilesExist verifies that migration 000024 ships both up
...
func TestMigration000024_FilesExist(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			filename: "000024_create_unique_idx_segment_ledger_name.up.sql",
		},
		{
			name:     "down migration file exists",
			filename: "000024_create_unique_idx_segment_ledger_name.down.sql",
		},
	}
...
// TestMigration000024_UpSQL_CreatesUniqueSegmentNameIndex verifies the up
// migration installs the per-ledger, case-insensitive uniqueness guarantee over
// live segment rows.
func TestMigration000024_UpSQL_CreatesUniqueSegmentNameIndex(t *testing.T) {
	t.Parallel()

	dir := migrationsDir(t)
	path := filepath.Join(dir, "000024_create_unique_idx_segment_ledger_name.up.sql")
	...
	tests := []struct {
		...
		{name: "names the index idx_segment_ledger_name_unique", substring: "idx_segment_ledger_name_unique", description: "must name the index idx_segment_ledger_name_unique"},
		{name: "targets the segment table", substring: "on segment", description: "must index the segment table"},
		{name: "keys on organization_id, ledger_id then lower(name)", substring: "(organization_id, ledger_id, lower(name))", description: "must key on organization_id, ledger_id then LOWER(name), in that order"},
...
// TestMigration000024_UpSQL_DoesNotBuildConcurrently guards the decision to
// build in-line: a CONCURRENTLY build that meets a duplicate leaves an INVALID
// index behind instead of aborting clean.
func TestMigration000024_UpSQL_DoesNotBuildConcurrently(t *testing.T) {
	t.Parallel()
...
// TestMigration000024_DownSQL_DropsIndex verifies the down migration removes
// the index added by the up migration.
func TestMigration000024_DownSQL_DropsIndex(t *testing.T) {
	t.Parallel()

	dir := migrationsDir(t)
	path := filepath.Join(dir, "000024_create_unique_idx_segment_ledger_name.down.sql")
	...
	tests := []struct {
		...
		{name: "targets idx_segment_ledger_name_unique", substring: "idx_segment_ledger_name_unique", description: "must drop the idx_segment_ledger_name_unique index"},
	}
