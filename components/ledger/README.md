...
- **Errors:** per route version — `/v1` serves the legacy `{title, message, code}` body at `application/json`; `>= /v2` serves RFC 9457 `application/problem+json`. Typed errors from `pkg/errors.go`, numeric
...
| **Shared platform** | lib-commons v6.2.0, lib-observability v2.1.0, lib-streaming v3.0.0 |
