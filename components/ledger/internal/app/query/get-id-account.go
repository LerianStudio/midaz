...
func (uc *UseCase) GetAccountByID(ctx context.Context, organizationID, ledgerID uuid.UUID, portfolioID *uuid.UUID, id uuid.UUID) (*a.Account, error) {
	...
	if err != nil {
		mlog.NewLoggerFromContext(ctx).Errorf("Error getting account on repo by id: %v", err)
	...
	if account != nil {
		...
		if err != nil {
			mlog.NewLoggerFromContext(ctx).Errorf("Error get metadata on mongodb account: %v", err)
