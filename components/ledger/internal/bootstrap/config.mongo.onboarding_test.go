...
func mustOnboardingTenantClient(t *testing.T, logger libLog.Logger) *tmclient.Client {
	t.Helper()
...
func TestInitOnboardingMongo(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := initOnboardingMongo(tt.opts, cfg, logger)
			if tt.wantErr {
				require.Error(t, err)
				assert.Nil(t, result)
...
func TestInitOnboardingMultiTenantMongo_Success(t *testing.T) {
	t.Parallel()
...
func TestInitOnboardingMultiTenantMongo_NilTenantClient_ReturnsError(t *testing.T) {
	t.Parallel()
...
func TestInitOnboardingSingleTenantMongo_CreatesComponents(t *testing.T) {
	t.Parallel()
	...
	result, err := initOnboardingSingleTenantMongo(cfg, logger)
	require.Error(t, err)
	assert.Nil(t, result)
}
