...
func FuzzBuildTransactionPostgresConnection_ConfigValues(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, host, port, user, password, dbname, sslmode string) {
		...
		assert.False(t, connected, "new connection must start disconnected")

		// Invariant 3: determinism -- calling again with the same config must
		// produce the same connection strings.
		conn2, err := buildTransactionPostgresConnection(cfg, logger)
		require.NoError(t, err)
		connected2, connectedErr2 := conn2.IsConnected()
		require.NoError(t, connectedErr2)
		assert.False(t, connected2, "new connection must start disconnected on repeat call")
	})
}
