...
func FuzzBuildOnboardingPostgresConnection_ConfigValues(f *testing.F) {
	...
	f.Add("localhost", "5432", "midaz_user", "s3cureP@ss!", "midaz_onboarding", "disable")
	...
	f.Add("localhost'; DROP TABLE organizations;--", "5432", "admin'--", "' OR '1'='1", "midaz'; DELETE FROM users;--", "disable")
	...
	f.Fuzz(func(t *testing.T, host, port, user, password, dbname, sslmode string) {
		...
		conn2, err := buildOnboardingPostgresConnection(cfg, logger)
		require.NoError(t, err)
