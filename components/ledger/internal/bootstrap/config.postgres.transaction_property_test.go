...
//   3. All 6 repository fields are always non-nil regardless of mode.
...
func sanitizeTransactionPropertyString(s string, maxLen int) string {
	if len(s) > maxLen {
		return s[:maxLen]
	}
...
func mustTransactionTenantClientFromLogger(logger libLog.Logger) *tmclient.Client {
...
func TestProperty_InitTransactionPostgres_MultiTenantAlwaysHasPGManager(t *testing.T) {
	...
	property := func(serviceName string) bool {
		serviceName = sanitizeTransactionPropertyString(serviceName, 256)
		...
		client := mustTransactionTenantClientFromLogger(logger)
		if client == nil {
			return false
		}
		...
		result, initErr := initTransactionPostgres(opts, cfg, logger)
		if initErr != nil {
			return false
		}
...
func TestProperty_InitTransactionPostgres_SingleTenantNeverHasPGManager(t *testing.T) {
	// Note: t.Parallel() removed because withTestConnector mutates package-level
	// postgresConnector, which is incompatible with parallel test execution.
...
// the tenancy mode (single or multi), all 6 repository fields in
...
func TestProperty_InitTransactionPostgres_AllReposAlwaysNonNil(t *testing.T) {
	// Note: t.Parallel() removed because withTestConnector mutates package-level
	// postgresConnector, which is incompatible with parallel test execution.
	...
	property := func(multiTenant bool, serviceName string) bool {
		...
		// Property: all 6 repos must be non-nil.
		return result.transactionRepo != nil &&
			result.operationRepo != nil &&
			result.assetRateRepo != nil &&
			result.balanceRepo != nil &&
			result.operationRouteRepo != nil &&
			result.transactionRouteRepo != nil
	}
...
func TestProperty_BuildTransactionPostgresConnection_NeverPanics(t *testing.T) {
	t.Parallel()

	logger := libLog.NewNop()

	property := func(host, port, user, password, dbname, sslmode string) bool {
		host = sanitizeTransactionPropertyString(host, 256)
		port = sanitizeTransactionPropertyString(port, 64)
		user = sanitizeTransactionPropertyString(user, 256)
		password = sanitizeTransactionPropertyString(password, 256)
		dbname = sanitizeTransactionPropertyString(dbname, 256)
