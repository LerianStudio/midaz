...
// Chaos tests for PostgreSQL bootstrap initialization in the onboarding module.
//
...
func TestIntegration_Chaos_InitPostgres_SingleTenantConnectionLoss(t *testing.T) {
	...
	require.NoError(t, dbErr, "Phase 1: Resolver must succeed")
	...
	chaos.AssertRecoveryWithin(t, func() error {
		recoveryResult, recoveryErr := initOnboardingSingleTenantPostgres(cfg, logger)
	...
	recoveredResult, err := initOnboardingSingleTenantPostgres(cfg, logger)
...
func TestIntegration_Chaos_InitPostgres_MultiTenantConnectionLoss(t *testing.T) {
	...
	tenantSrv := httptest.NewServer(stdhttp.HandlerFunc(func(w stdhttp.ResponseWriter, _ *stdhttp.Request) {
	...
	chaos.AssertRecoveryWithin(t, func() error {
		recoveryResult, recoveryErr := initOnboardingMultiTenantPostgres(opts, cfg, logger)
		if recoveryResult != nil {
			closePGConnection(recoveryResult.connection)
		}
	...
	recoveredResult, err := initOnboardingMultiTenantPostgres(opts, cfg, logger)
...
// subsequent Resolver/query operations on the repos return errors gracefully
...
func TestIntegration_Chaos_InitPostgres_PostInitConnectionLoss(t *testing.T) {
	...
	require.NotPanics(t, func() {
		...
		if freshErr != nil {
			t.Logf("Phase 3: Resolver returned error: %v", freshErr)
			return
		}
	...
	}, "Phase 3: Resolver + query must not panic when PG is down")
