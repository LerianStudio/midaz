...
// Integration tests for PostgreSQL bootstrap initialization in the transaction module.
//
...
// Run with:
//
//	go test -tags integration -v -run TestIntegration_InitPostgres ./components/transaction/internal/bootstrap/...
package bootstrap
...
func TestIntegration_InitTransactionPostgres_SingleTenantProducesWorkingRepos(t *testing.T) {
	...
	assert.NotNil(t, result.transactionRepo, "transactionRepo must be initialized")
	assert.NotNil(t, result.operationRepo, "operationRepo must be initialized")
	assert.NotNil(t, result.assetRateRepo, "assetRateRepo must be initialized")
	assert.NotNil(t, result.balanceRepo, "balanceRepo must be initialized")
	assert.NotNil(t, result.operationRouteRepo, "operationRouteRepo must be initialized")
...
func TestIntegration_InitTransactionPostgres_MultiTenantProducesWorkingRepos(t *testing.T) {
	infra := setupBootstrapIntegrationInfra(t)
...
func TestIntegration_InitTransactionPostgres_BuildConnectionProducesConnectable(t *testing.T) {
	infra := setupBootstrapIntegrationInfra(t)
...
// in both cases.
func TestIntegration_InitTransactionPostgres_DispatcherRoutesCorrectly(t *testing.T) {
	infra := setupBootstrapIntegrationInfra(t)
	...
	t.Run("nil opts routes to single-tenant", func(t *testing.T) {
		result, err := initTransactionPostgres(nil, cfg, logger)
	...
	t.Run("multi-tenant opts routes to multi-tenant", func(t *testing.T) {
		...
		result, err := initTransactionPostgres(opts, cfg, logger)
...
func TestIntegration_InitTransactionPostgres_PrefixedFallbackConnects(t *testing.T) {
	infra := setupBootstrapIntegrationInfra(t)
...
func TestIntegration_InitTransactionPostgres_InvalidConfigReturnsError(t *testing.T) {
	logger, err := libZap.New(libZap.Config{Environment: libZap.EnvironmentDevelopment, OTelLibraryName: "midaz-tests"})
	require.NoError(t, err)
...
func TestIntegration_InitTransactionPostgres_MultiTenantInvalidConfigReturnsError(t *testing.T) {
	logger, err := libZap.New(libZap.Config{Environment: libZap.EnvironmentDevelopment, OTelLibraryName: "midaz-tests"})
	require.NoError(t, err)
