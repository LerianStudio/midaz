...
func TestIntegration_InitOnboardingPostgres_DispatcherRoutesCorrectly(t *testing.T) {
	...
	t.Run("nil opts routes to single-tenant", func(t *testing.T) {
		result, err := initOnboardingPostgres(nil, cfg, logger)
	...
	t.Run("multi-tenant opts routes to multi-tenant", func(t *testing.T) {
		...
		prev := onboardingPostgresConnector
		onboardingPostgresConnector = defaultOnboardingPostgresConnector
		t.Cleanup(func() { onboardingPostgresConnector = prev })
		...
		result, err := initOnboardingPostgres(opts, cfg, logger)
...
func TestIntegration_InitOnboardingPostgres_InvalidConfigReturnsError(t *testing.T) {
	logger, err := libZap.New(libZap.Config{Environment: libZap.EnvironmentDevelopment, OTelLibraryName: "midaz-tests"})
	require.NoError(t, err)
...
func TestIntegration_InitOnboardingPostgres_MultiTenantInvalidConfigReturnsError(t *testing.T) {
	logger, err := libZap.New(libZap.Config{Environment: libZap.EnvironmentDevelopment, OTelLibraryName: "midaz-tests"})
	require.NoError(t, err)
