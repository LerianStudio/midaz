...
func TestIntegration_OwnedLegacyFence_RedisClusterRejectsCrossSlotButAcceptsCompanionScript(t *testing.T) {
	...
	admitted, leaseHeld, phase, err := guard.AcquireRevert(ctx, "legacy", "cluster-phase-zero-revert")
	require.NoError(t, err)
	assert.True(t, admitted)
	assert.True(t, leaseHeld)
	assert.Equal(t, RevertUpdateFreezeActive, phase)
	require.Error(t, guard.MarkPhaseZeroDrained(ctx))
	require.NoError(t, guard.ReleaseRevert(ctx, "legacy", "cluster-phase-zero-revert"))
...
func TestIntegration_RevertUpdateFreezeMarkerIsSharedPersistentAndFinalizable(t *testing.T) {
	...
	legacyRevertAdmitted, legacyRevertLease, legacyRevertPhase, err := guard.AcquireRevert(ctx, "legacy", "phase-zero-revert")
	require.NoError(t, err)
	...
	otherLegacyAdmitted, otherLegacyLease, otherLegacyPhase, err := guard.AcquireRevert(ctx, "legacy", "other-phase-zero-revert")
	require.NoError(t, err)
	...
	require.NoError(t, guard.ReleaseRevert(ctx, "legacy", "phase-zero-revert"))
	require.Error(t, guard.MarkPhaseZeroDrained(ctx), "owner cleanup must not remove another request's rollout lease")
	require.NoError(t, guard.ReleaseRevert(ctx, "legacy", "other-phase-zero-revert"))
	...
	bridgeRevertAdmitted, bridgeRevertLease, bridgeRevertPhase, err := guard.AcquireRevert(ctx, "bridge", "bridge-revert")
	require.NoError(t, err)
	...
	require.NoError(t, guard.ReleaseRevert(ctx, "bridge", "bridge-revert"))
