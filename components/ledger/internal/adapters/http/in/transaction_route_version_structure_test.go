...
// the tracer.
//
// Gate 5 (the route gate must be the FIRST statement of each tracer seam) lives beside
// the seams themselves, in the command package.
...
func TestRouteVersionStructure_V1StateRoutesPassRouteV1(t *testing.T) {
	...
	for _, callee := range []string{"commitTransaction", "revertTransaction"} {
		...
		for i, got := range policies {
			if got != "RouteV1" {
				t.Errorf("Gate 6: %s call #%d in transaction_handler.go passes %q, want command.RouteV1 — the /v1 contract carries no tracer", callee, i, got)
			}
		}
	}
}
...
func TestRouteVersionStructure_V2StateRoutesPassRouteV2(t *testing.T) {
	...
	for _, callee := range []string{"commitTransaction", "revertTransaction"} {
		...
		for i, got := range policies {
			if got != "RouteV2" {
				t.Errorf("Gate 6: %s call #%d in transaction_handler_v2.go passes %q, want command.RouteV2", callee, i, got)
			}
		}
	}
}
