...
// list + alias + external + count set is the whole surface. The v2 twin is a STRAIGHT
// MIRROR — same handler method, same input/output types — so its operationId is the v1
// id with the version suffix appended. That suffix is the only thing that keeps the two
...
var accountV2Ops = []struct {
	...
	{action: "create", method: http.MethodPost, opPath: "/organizations/{organization_id}/ledgers/{ledger_id}/accounts", v1OperationID: "createAccount"},
	...
	{action: "getByID", method: http.MethodGet, opPath: "/organizations/{organization_id}/ledgers/{ledger_id}/accounts/{id}", v1OperationID: "getAccountByID"},
	{action: "getByAlias", method: http.MethodGet, opPath: "/organizations/{organization_id}/ledgers/{ledger_id}/accounts/alias/{alias}", v1OperationID: "getAccountByAlias"},
	{action: "getExternalByCode", method: http.MethodGet, opPath: "/organizations/{organization_id}/ledgers/{ledger_id}/accounts/external/{code}", v1OperationID: "getAccountExternalByCode"},
	{action: "update", method: http.MethodPatch, opPath: "/organizations/{organization_id}/ledgers/{ledger_id}/accounts/{id}", v1OperationID: "updateAccount"},
...
// and /v2 twins; a straight mirror names the identical set on each side.
...
// TestRegisterAccountV2Routes_ReusesV1SchemaComponents proves the core correctness claim of
// the straight-mirror approach: the /v2 account twin REUSES the v1 request/response Go types,
...
func TestRegisterAccountV2Routes_ReusesV1SchemaComponents(t *testing.T) {
	t.Parallel()
	...
	// came back "": at least one account op must actually name a response-body component.
	...
	for _, op := range accountV2Ops {
		...
		assert.Equalf(t, v1Req, v2Req,
			"the v2 %s account op must name the SAME request-body schema as v1 (a straight mirror mints no new request type)", op.action)
		assert.ElementsMatchf(t, v1Resp, v2Resp,
			"the v2 %s account op must name the SAME response-body component(s) as v1 (Huma dedups the reused Go type to one schema)", op.action)
		...
		for _, ref := range v2Resp {
			...
			assert.Falsef(t, strings.HasSuffix(ref, accountV2OperationSuffix),
				"the v2 %s account op response ref %q must not name a %s-suffixed component — the v1 type is reused, not re-minted",
				op.action, ref, accountV2OperationSuffix)
		}
	}

	require.True(t, sawSharedResponseRef,
		"at least one account op must reference a response-body component, or the reuse claim is vacuous")
}

// TestRegisterAccountV2Routes_MintsNoV2SchemaComponents guards against accidental new-type
// creation for the straight mirror: no account schema component may carry the version suffix.
// TransactionV2 is a legitimate component (transaction v2 is NOT a straight mirror and DOES
// introduce its own types); the account mirror must add no such twin.
func TestRegisterAccountV2Routes_MintsNoV2SchemaComponents(t *testing.T) {
	t.Parallel()
	...
	// rather than hardcoded, so a renamed account type is followed here. The reused v1 type
	...
	for name := range referenced {
		assert.NotContainsf(t, schemas, name+accountV2OperationSuffix,
			"no %s twin of the reused account body component %q may be minted", accountV2OperationSuffix, name)
	}

	// The document-wide guard: no account-named schema carries the V2 suffix. The prefix
	// also spans the sibling Account* families (AccountType, AccountRule); none is a straight-
	// mirror V2 twin, so all must clear the suffix check.
	for name := range schemas {
		...
		assert.Falsef(t, strings.HasSuffix(name, accountV2OperationSuffix),
			"no account schema component may carry the %s suffix; found %q", accountV2OperationSuffix, name)
	}
}
