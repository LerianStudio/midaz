...
// TestRegisterAssetV2Routes_NoDuplicateOperationIDs asserts every operationId in the assembled
// document is unique. huma.OpenAPI.AddOperation panics on a duplicate id, so the served
// contract cannot boot with two ops sharing one — this pins the invariant at the document
// level, catching a v2 twin that dropped its suffix before it becomes a boot panic.
func TestRegisterAssetV2Routes_NoDuplicateOperationIDs(t *testing.T) {
	t.Parallel()
