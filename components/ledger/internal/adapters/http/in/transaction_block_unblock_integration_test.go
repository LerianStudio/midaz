...
// ID the resolver looks up.
func buildBlockUnblockRouteCache(t *testing.T, opType string, withBlock, withUnblock bool) (*mmodel.TransactionRouteCache, string) {
	t.Helper()
	...
	opRouteID := uuid.Must(libCommons.GenerateUUIDv7())
...
// TestIntegration_BlockUnblock_RubricResolution validates, against a real
// Postgres/Mongo/Redis stack, that BuildOperations routes a block/unblock
// operation to its dedicated accounting rubric when the operation route
// configures one (via the production ToCache() cache shape), and falls back to
// the Direct rubric when it does not (Phase 1 parity). This exercises the
// resolvedAction override + resolveAccountingRubric Block/Unblock cases through
// the same path the create handler uses.
func TestIntegration_BlockUnblock_RubricResolution(t *testing.T) {
	infra := setupBlockUnblockInfra(t)
	infra.seedLedgerSettings(t)

	newOp := func(routeID, direction string) *operation.Operation {
		id := routeID
		...
		return &operation.Operation{
			...
			RouteID:    &id,
			Direction:  direction,
			BalanceKey: cn.DefaultBalanceKey,
		}
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			cache, routeID := buildBlockUnblockRouteCache(t, tt.opType, tt.withBlock, tt.withUnblock)

			op := newOp(routeID, tt.direction)
			ops := []*operation.Operation{op}

			resolveRouteCodesFromCache(ops, cache, cn.ActionDirect, tt.override)
