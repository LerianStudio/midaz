...
func RegisterHolderV2RoutesToApp(group fiber.Router, api huma.API, auth *middleware.AuthClient, h *HolderHandler, routeOptions, ledgerReadOptions *pkgHTTP.ProtectedRouteOptions) {
	registerHolderRoutesToApp(group, api, auth, h, routeOptions, ledgerReadOptions, v2OpSuffix)
}
...
//
// Delete carries ledgerReadOptions because its owned-account guard counts the holder's
...
func registerHolderRoutesToApp(group fiber.Router, api huma.API, auth *middleware.AuthClient, h *HolderHandler, routeOptions, ledgerReadOptions *pkgHTTP.ProtectedRouteOptions, opSuffix string) {
	const (
	...
	routeDelete(group, holderIDPath, protectedMidaz(auth, "holders", "delete", ledgerReadOptions, holderParse))
