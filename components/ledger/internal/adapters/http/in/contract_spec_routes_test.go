...
// share one component registry, so the union is a single spec — 104 path keys (72
// under /v1, 32 under /v2), 168 operations — served on the single root server "/".
func TestUnifiedHumaContractIsSingleDocument(t *testing.T) {
	...
	require.Len(t, doc.Paths, 104, "single document must enumerate every versioned path key")
	require.Equal(t, 72, v1, "path keys under /v1")
	require.Equal(t, 32, v2, "path keys under /v2")
	require.Equal(t, 168, ops, "operations across both versions")
