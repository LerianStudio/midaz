...
// share one component registry, so the union is a single spec — 102 path keys (72
// under /v1, 30 under /v2), 163 operations — served on the single root server "/".
func TestUnifiedHumaContractIsSingleDocument(t *testing.T) {
	...
	require.Len(t, doc.Paths, 102, "single document must enumerate every versioned path key")
	require.Equal(t, 72, v1, "path keys under /v1")
	require.Equal(t, 30, v2, "path keys under /v2")
	require.Equal(t, 163, ops, "operations across both versions")
