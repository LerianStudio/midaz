...
// share one component registry, so the union is a single spec — 136 path keys (62
// under /v1, 74 under /v2), 217 operations — served on the single root server "/".
// CRM is /v2-only, so its keys and operations count toward /v2 alone.
func TestUnifiedHumaContractIsSingleDocument(t *testing.T) {
	...
	require.Len(t, doc.Paths, 136, "single document must enumerate every versioned path key")
	require.Equal(t, 62, v1, "path keys under /v1")
	require.Equal(t, 74, v2, "path keys under /v2")
	require.Equal(t, 217, ops, "operations across both versions")
