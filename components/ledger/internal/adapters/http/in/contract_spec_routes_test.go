...
// share one component registry, so the union is a single spec — 98 path keys (72
// under /v1, 26 under /v2), 155 operations — served on the single root server "/".
func TestUnifiedHumaContractIsSingleDocument(t *testing.T) {
	...
	require.Len(t, doc.Paths, 98, "single document must enumerate every versioned path key")
	require.Equal(t, 72, v1, "path keys under /v1")
	require.Equal(t, 26, v2, "path keys under /v2")
	require.Equal(t, 155, ops, "operations across both versions")
