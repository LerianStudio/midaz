...
// share one component registry, so the union is a single spec — 130 path keys (56
// under /v1, 74 under /v2), 205 operations — served on the single root server "/".
// CRM and fees/billing are /v2-only, so their keys and operations count toward /v2 alone.
func TestUnifiedHumaContractIsSingleDocument(t *testing.T) {
	...
	require.Equal(t, 56, v1, "path keys under /v1")
	require.Equal(t, 74, v2, "path keys under /v2")
	require.Equal(t, 205, ops, "operations across both versions")
