...
func TestErrorEnvelope_AccountClosingCodesCrossVersions(t *testing.T) {
	problem := `{"type":"https://errors.lerian.studio/v1/0514","title":"Account Already Closed Error",` +
		`"status":409,"detail":"The account is already closed.","code":"0514","entityType":"Account"}`
	...
	t.Run("v1 keeps the code while the shape changes", func(t *testing.T) {
		...
		assert.JSONEq(t,
			`{"title":"Account Already Closed Error","message":"The account is already closed.","code":"0514"}`,
			body)
		assert.Contains(t, contentType, fiber.MIMEApplicationJSON)
	})
}
