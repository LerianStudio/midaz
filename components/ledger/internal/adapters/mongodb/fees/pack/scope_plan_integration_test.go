...
// TestIntegration_PackageScopeFilter_ResolvesBySingleDocumentFetch pins the
// access path behind every by-ID package read and write.
//
// packageScopeFilter always pins _id, and _id is backed by the unique index
// MongoDB creates on every collection. That makes the lookup resolve to at most
// one document before organization_id, ledger_id and deleted_at are applied as
// residual predicates, under BOTH scope shapes the helper emits. The counts are
// asserted rather than the index name so the test states the guarantee that
// matters — a by-ID lookup on the money path never scans — instead of pinning
// one planner's choice.
//
...
func TestIntegration_PackageScopeFilter_ResolvesBySingleDocumentFetch(t *testing.T) {
	...
	coll := packCollection(container)

	organizationID := uuid.New()
	ownerLedgerID := uuid.New()
	otherLedgerID := uuid.New()
	targetID := uuid.New()
	...
	for i := range scopePlanSeedCount {
		id, ledgerID := uuid.New(), otherLedgerID
		if i%2 == 0 {
	...
	require.NoError(t, err, "seeding the package collection must succeed")
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			filter := packageScopeFilter(targetID, organizationID, tt.ledgerID)
