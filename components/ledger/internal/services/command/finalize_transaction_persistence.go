...
//nolint:gocognit,gocyclo // Durable finalization reconciles every pending persistence leg; refactor candidate.
...
func (uc *UseCase) completeDurableRevertReplay(
	...
	if claim.LegacyFenceKey != nil && *claim.LegacyFenceKey != "" {
		...
		if err != nil && strings.EqualFold(strings.TrimSpace(uc.RevertIdempotencyMode), constant.RevertRolloutModeFinal) {
			...
			if readErr == nil {
				fenceOwner := values[*claim.LegacyFenceKey+":owner"]
				value := values[*claim.LegacyFenceKey]
				foreignOwner := fenceOwner != "" && fenceOwner != claim.ReverseTransactionID.String()
