...
func (uc *UseCase) UpdateMetadataOnboarding(ctx context.Context, entityName, entityID string, metadata map[string]any) (map[string]any, error) {
	...
	if metadataToUpdate != nil {
		existingMetadata, err := uc.MetadataOnboardingRepo.FindByEntity(ctx, entityName, entityID)
		if err != nil {
	...
	if err := uc.MetadataOnboardingRepo.Update(ctx, entityName, entityID, metadataToUpdate); err != nil {
