...
type operationLifecycle struct {
	Before accounting.BalanceState
...
func BuildOperationRecordsFromMovements(payload TransactionCompletionPlan, result accounting.ExecutionResult) ([]*operation.Operation, error) {
...
func projectValidatedCancelRelease(context OperationRecordSpec, before accounting.BalanceState) (accounting.BalanceState, accounting.BalanceState, decimal.Decimal) {
	after := before
...
func projectValidatedCancelCredit(context OperationRecordSpec, initial accounting.BalanceState) (accounting.BalanceState, accounting.BalanceState, decimal.Decimal) {
...
func operationLifecycles(payload TransactionCompletionPlan, movements []accounting.Movement) map[operationMovementKey]operationLifecycle {
...
func validateOperationMovementResult(payload TransactionCompletionPlan, result accounting.ExecutionResult) (map[operationMovementKey]accounting.Movement, error) {
...
func validateOperationMovement(movement accounting.Movement, requested decimal.Decimal) error {
	...
	for key, movement := range movements {
		if movement.Role == accounting.RoleOverdraftCompanion {
			...
			if primary.OverdraftDelta.IsPositive() {
				expectedType = accounting.PostingDebit
			}
		...
		companion, exists := movements[operationMovementKey{PostingRef: key.PostingRef, Role: accounting.RoleOverdraftCompanion, Ordinal: key.Ordinal}]
...
func validateOperationFinal(finals []accounting.BalanceSnapshot, last map[string]accounting.BalanceState, identities map[string]OperationBalanceContext) error {
	...
	for _, final := range finals {
		...
		actual := accounting.BalanceState{Available: final.Available, OnHold: final.OnHold, OverdraftUsed: final.OverdraftUsed, Version: final.Version}
...
func validOperationRecordPostingType(postingType accounting.PostingType) bool {
	switch postingType {
...
func sameOperationState(left, right accounting.BalanceState) bool {
