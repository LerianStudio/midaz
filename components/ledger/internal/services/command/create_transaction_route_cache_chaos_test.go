...
// Run with:
//
//	CHAOS=1 go test -tags integration -v -run TestIntegration_Chaos ./components/ledger/internal/services/command/...
package command
...
func TestIntegration_Chaos_Redis_ConnectionLoss_CreateAccountingRouteCache(t *testing.T) {
	...
	internalKey := utils.AccountingRoutesInternalKey(route.OrganizationID, route.ID)
	...
	recoveryKey := utils.AccountingRoutesInternalKey(recoveryRoute.OrganizationID, recoveryRoute.ID)
