...
dedicated `("dashboard","get")` — borrowing `transactions` would widen it to
disclose balance positions. **That pair is not yet in the caradhras seed**, so a
...
up**: rounding the end down hides up to 60 s of the most recent activity, which
a console reads as "nothing happened". `updatedAt` says when it was computed.
...
expected. Statuses come from `constant.TransactionStatuses`; the settled set is
a SQL parameter, so a new status needs no change here.
...
`scale`. If a scale column returns, so does the division, per row, pre-sum. It
counts only accounts that HOLD money: `@external/<asset>` is the contra side of
every issuance, so summing it reports roughly twice the position.
...
through to Postgres and logs; never fatal.
...
at 155k, 181/205 at 332k, 586/735 at 996k. `/volume` binds, crossing 200 ms at
**~320k rows in the window**: about **46,000 transactions/day at 7d, 10,700 at
30d, 3,600 at 90d** (`/metrics` at ~365k). Past that the 60 s cache is the
protection: one aggregation per ledger per minute at any viewer count. `/assets`
is bounded by account count, not history — 63 ms at 50k balances, 320 ms at
200k, crossing at **~125k**.
...
matters, 382 MB beside a 440 MB heap, ~+6% insert cost, and moves only `/volume`
at 30d over the line.

**Out of scope, deliberately**: `/top-accounts`, operations-level counts, and
distinct accounts touched — each needs a new index on `operation` or an
outbox-fed rollup, plus a backfill story.
