...
// fakeRunner is a no-op transaction service used for the happy-path test. The
// real service.Run() blocks on Fiber/gRPC/consumer goroutines; the fake lets
// run() return promptly without spinning up any infrastructure.
...
func TestRun_Healthcheck_ShortCircuits(t *testing.T) {
	...
	buf := &bytes.Buffer{}
	// Intentionally leave all deps panicking. --healthcheck must return 0
...
// TestRealDeps_AllFieldsPopulated verifies realDeps() returns a fully-populated
// deps struct. We cannot call the closures (they would touch real env/logger/DB),
// but we can assert they are non-nil so main() never dereferences a zero dep.
...
// TestRealDeps_InitEnvConfig_ReturnsNil locks in the contract that our
// env-config closure returns nil. InitLocalEnvConfig is void in libCommons; a
// future refactor must not silently start returning errors without main()
...
// configured it falls back to defaults and returns successfully.
...
// TestRealDeps_InitService_IsWiredButNotInvoked documents that the service-init
// closure is intentionally NOT exercised end-to-end in unit tests — it would
// try to open real PostgreSQL/MongoDB/Redpanda/gRPC connections. This test
// asserts only that the closure is non-nil; integration tests cover happy path.
...
// TestRealDeps_InitService_ExecutesClosureBody exercises the production
// initService closure just far enough to cover its statements. We feed it a
// NoneLogger; the closure will call bootstrap.InitServersWithOptions, which
// will almost certainly return an error in a unit-test environment (no
