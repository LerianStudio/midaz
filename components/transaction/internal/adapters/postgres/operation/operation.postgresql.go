...
// CreateManyAndApplyDeltasAtomic inserts multiple operations and applies their balance deltas atomically.
// It performs idempotent inserts (ON CONFLICT DO NOTHING) and only applies deltas for operations actually inserted now.
// If applyBalances is false, operations are inserted but balances are not updated.
func (r *OperationPostgreSQLRepository) CreateManyAndApplyDeltasAtomic(
	ctx context.Context,
	...
	ctx, span := tracer.Start(ctx, "postgres.create_many_operations_and_apply_deltas_atomic")
	defer span.End()
	...
	for attempt = 0; attempt < 5; attempt++ {
		...
		for _, op := range operations {
			...
			result, err := tx.ExecContext(ctx, query,
				...
				rec.OnHoldBalance,
				rec.AvailableBalanceAfter,
				rec.OnHoldBalanceAfter,
				rec.Status,
			...
			if rows > 0 && applyBalances {
				if op.BalanceAffected && op.Balance.Available != nil && op.BalanceAfter.Available != nil && op.Balance.OnHold != nil && op.BalanceAfter.OnHold != nil {
					da := op.BalanceAfter.Available.Sub(*op.Balance.Available)
					dh := op.BalanceAfter.OnHold.Sub(*op.Balance.OnHold)
					cur := deltas[op.BalanceID]
					cur.da = cur.da.Add(da)
					cur.dh = cur.dh.Add(dh)
					deltas[op.BalanceID] = cur
				}
			}
		}
