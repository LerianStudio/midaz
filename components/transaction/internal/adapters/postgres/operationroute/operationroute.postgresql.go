...
func scanOperationRouteRows(rows *sql.Rows) ([]*mmodel.OperationRoute, map[uuid.UUID]bool, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			return nil, nil, pkg.ValidateInternalError(err, "OperationRoute")
		}
...
func calculateOperationRoutePagination(operationRoutes []*mmodel.OperationRoute, filter http.Pagination, decodedCursor libHTTP.Cursor, hasPagination bool) (libHTTP.CursorPagination, error) {
	...
	if err != nil {
		return libHTTP.CursorPagination{}, pkg.ValidateInternalError(err, "OperationRoute")
	}
...
func (r *OperationRoutePostgreSQLRepository) HasTransactionRouteLinks(ctx context.Context, operationRouteID uuid.UUID) (bool, error) {
	...
	if err != nil {
		...
		return false, pkg.ValidateInternalError(err, "OperationRoute")
	}
