...
//
// These fuzz tests verify that the two opaque-handle getters on Service
// (GetPGManager, GetMongoManager) never panic regardless of what value is
// stored in the underlying field.
//
...
// an opaqueHandle (which satisfies interface{}) and then calls both getters,
// verifying they never panic and always return the stored value.
//
// Seed corpus covers:
...
func FuzzServiceGetters_NeverPanic(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, payload []byte) {
		...
		// --- Case A: non-nil handle stored in both fields ---
		...
		// Both getters must not panic and must return the exact stored value.
		...
		// --- Case B: nil stored in both fields (single-tenant mode) ---
		svcNil := &Service{
			pgManager:    nil,
			mongoManager: nil,
		}
