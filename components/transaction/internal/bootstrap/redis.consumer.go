...
func (r *RedisQueueConsumer) readMessagesAndProcess(ctx context.Context) {
	...
	for key, message := range messages {
		...
		go r.processMessageAsync(ctx, tracer, key, transaction, sem, &wg)
	}
...
func (r *RedisQueueConsumer) processMessageAsync(
	ctx context.Context,
	tracer trace.Tracer,
	key string,
	m mmodel.TransactionRedisQueue,
	sem chan struct{},
	wg *sync.WaitGroup,
) {
	defer func() {
...
func (r *RedisQueueConsumer) processMessage(ctx context.Context, tracer trace.Tracer, key string, m mmodel.TransactionRedisQueue) { //nolint:gocyclo,cyclop // message processing; complexity is inherent
	...
	if m.Validate == nil {
		logger.Warnf("Message (key: %s) has nil Validate field, removing malformed backup entry", key)
