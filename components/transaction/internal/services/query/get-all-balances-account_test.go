...
func TestGetAllBalancesByAccountID(t *testing.T) { //nolint:funlen
	...
	t.Run("SuccessWithRedisOverlayUsesShardKeyWhenShardRouterConfigured", func(t *testing.T) {
		t.Parallel()
		ctrl := gomock.NewController(t)
		t.Cleanup(ctrl.Finish)
		...
		shardID := router.Resolve(item.Alias)
		shardKey := utils.BalanceShardKey(shardID, organizationID, ledgerID, item.Alias+"#"+item.Key)
		cachePayload := `{"available":"77.10","onHold":"9","version":99}`

		mockRedisRepo.
			EXPECT().
			MGet(gomock.Any(), gomock.Eq([]string{shardKey})).
			Return(map[string]string{shardKey: cachePayload}, nil).
			Times(1)
