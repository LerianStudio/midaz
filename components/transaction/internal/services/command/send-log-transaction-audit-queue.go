...
func (uc *UseCase) SendLogTransactionAuditQueue(ctx context.Context, operations []*operation.Operation, organizationID, ledgerID, transactionID uuid.UUID) {
	...
	for i, o := range operations {
		...
		if err != nil {
			...
			logger.Errorf("Failed to marshal operation to JSON string: %s", err)
		...
		if err != nil {
			...
			logger.Errorf("Invalid operation UUID for audit payload: %s", err)
	...
	if _, err := uc.BrokerRepo.ProducerDefault(
		...
		logger.Errorf("Failed to send audit message: %s", err)
	}
}
