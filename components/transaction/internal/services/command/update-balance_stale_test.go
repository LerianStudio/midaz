...
func TestUpdateBalances_AllStale_ReturnsError(t *testing.T) {
	...
	// Assert: Error wraps ErrStaleBalanceUpdateSkipped
	assert.True(t, errors.Is(err, constant.ErrStaleBalanceUpdateSkipped),
		"Error should wrap ErrStaleBalanceUpdateSkipped, got: %v", err)

	// Assert: Error message is descriptive
	assert.Contains(t, err.Error(), "balance updates skipped due to stale versions",
