...
func TestSyncBalance_SuccessSynced(t *testing.T) {
	...
	balanceRedis := mmodel.BalanceRedis{ID: uuid.Must(libCommons.GenerateUUIDv7()).String(), Alias: "@alias"}
...
func TestSyncBalance_SuccessSkipped(t *testing.T) {
	organizationID := uuid.Must(libCommons.GenerateUUIDv7())
	ledgerID := uuid.Must(libCommons.GenerateUUIDv7())
	balanceRedis := mmodel.BalanceRedis{ID: uuid.Must(libCommons.GenerateUUIDv7()).String(), Alias: "@alias2"}
...
func TestSyncBalance_Error(t *testing.T) {
	organizationID := uuid.Must(libCommons.GenerateUUIDv7())
	ledgerID := uuid.Must(libCommons.GenerateUUIDv7())
	balanceRedis := mmodel.BalanceRedis{ID: uuid.Must(libCommons.GenerateUUIDv7()).String(), Alias: "@alias3"}
