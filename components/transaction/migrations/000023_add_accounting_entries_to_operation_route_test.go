...
func TestMigration000023_FilesExist(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			filename: "000023_add_accounting_entries_to_operation_route.up.sql",
		},
		{
			name:     "down migration file exists",
			filename: "000023_add_accounting_entries_to_operation_route.down.sql",
		},
	}
...
func TestMigration000023_UpSQL_AddAccountingEntriesColumn(t *testing.T) {
	t.Parallel()

	dir := migrationsDir(t)
	path := filepath.Join(dir, "000023_add_accounting_entries_to_operation_route.up.sql")
...
func TestMigration000023_DownSQL_DropAccountingEntriesColumn(t *testing.T) {
	t.Parallel()

	dir := migrationsDir(t)
	path := filepath.Join(dir, "000023_add_accounting_entries_to_operation_route.down.sql")
...
func TestMigration000023_UpSQL_IdempotentRerun(t *testing.T) {
	t.Parallel()
...
func TestMigration000023_UpSQL_ExistingRowsUnaffected(t *testing.T) {
	t.Parallel()
