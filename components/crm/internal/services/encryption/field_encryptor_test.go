...
// TestFieldEncryptor_EncryptOptional verifies EncryptOptional helper method
// that handles nil values gracefully for optional fields.
func TestFieldEncryptor_EncryptOptional(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:      "encrypts when value is present",
			value:     strPtr("test-value"),
			expectNil: false,
			wantErr:   false,
		},
		{
			name:      "encrypts empty string",
			value:     strPtr(""),
			expectNil: false,
			wantErr:   false,
		},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			mockSvc := &mockEncryptionService{
				encryptFunc: func(_ context.Context, _ FieldContext, plaintext string) (string, error) {
					return "encrypted:" + plaintext, nil
				},
			}
			...
			result, err := adapter.EncryptOptional(ctx, fieldCtx, tc.value)
			...
			if tc.wantErr {
				...
				if tc.expectNil {
					...
					assert.Equal(t, "encrypted:"+*tc.value, *result)
				}
			}
		})
	}
...
// TestFieldEncryptor_DecryptOptional verifies DecryptOptional helper method
// that handles nil values gracefully for optional fields.
func TestFieldEncryptor_DecryptOptional(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:      "decrypts when value is present",
			value:     strPtr("encrypted:test-value"),
			expected:  strPtr("test-value"),
			expectNil: false,
			wantErr:   false,
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			mockSvc := &mockEncryptionService{
				decryptFunc: func(_ context.Context, _ FieldContext, ciphertext string) (string, error) {
					if len(ciphertext) > 10 && ciphertext[:10] == "encrypted:" {
						return ciphertext[10:], nil
					}
					return ciphertext, nil
			...
			result, err := adapter.DecryptOptional(ctx, fieldCtx, tc.value)
