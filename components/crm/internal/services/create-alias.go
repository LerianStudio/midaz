...
func (uc *UseCase) CreateAlias(ctx context.Context, organizationID string, holderID uuid.UUID, cai *mmodel.CreateAliasInput) (*mmodel.Alias, error) {
	...
	uc.emitAliasCreatedEvent(ctx, span, logger, createdAlias, organizationID)

	return createdAlias, nil
}

func (uc *UseCase) emitAliasCreatedEvent(ctx context.Context, span trace.Span, logger libLog.Logger, a *mmodel.Alias, organizationID string) {
	pkgStreaming.EmitImportant(ctx, span, logger, uc.Streaming, events.AliasCreatedDefinition.Key(),
		func(tenantID string) (libStreaming.EmitRequest, error) {
			return events.NewAliasCreated(a, organizationID).ToEmitRequest(tenantID, a.CreatedAt)
		})
}
