...
// Activate handles the activation of an organization's envelope encryption.
//
...
//	@Param			Authorization		header		string							false	"The authorization token in the 'Bearer	access_token' format. Only required when auth plugin is enabled."
//	@Param			organization_id		path		string							true	"The unique identifier of the Organization."
//	@Param			input				body		mmodel.ActivateEncryptionInput	true	"Activate Input"
//	@Success		200					{object}	mmodel.ActivateEncryptionResponse
//	@Failure		400					{object}	pkg.HTTPError
//	@Failure		404					{object}	pkg.HTTPError
//	@Failure		500					{object}	pkg.HTTPError
//	@Router			/v1/organizations/{organization_id}/encryption/activate [post]
func (handler *EncryptionHandler) Activate(p any, c *fiber.Ctx) error {
	ctx := c.UserContext()

	logger, tracer, reqId, _ := libCommons.NewTrackingFromContext(ctx)

	ctx, span := tracer.Start(ctx, "handler.activate_encryption")
	defer span.End()
	...
	payload, ok := p.(*mmodel.ActivateEncryptionInput)
	if !ok || payload == nil {
		return http.WithError(c, cn.ErrInternalServer)
	}
	...
	input := encryption.ActivateInput{
		OrganizationID: organizationID.String(),
		Actor:          payload.Actor,
		Reason:         payload.Reason,
	}

	if err := handler.ProvisioningService.Activate(ctx, input); err != nil {
		libOpenTelemetry.HandleSpanError(span, "Failed to activate encryption", err)

		logger.Log(ctx, libLog.LevelError, "Failed to activate encryption", libLog.Err(err))

		return http.WithError(c, err)
	}

	response := mmodel.ActivateEncryptionResponse{
		OrganizationID: organizationID.String(),
		Status:         string(mmodel.RegistryStatusActive),
	}
