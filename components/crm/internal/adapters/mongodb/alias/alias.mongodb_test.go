...
func TestMongoDBRepository_appendEncryptedFilters_UsesInOperator(t *testing.T) {
	...
	query := http.QueryHeader{
		...
		BankingDetailsAccount:               &account,
		BankingDetailsIban:                  &iban,
		RegulatoryFieldsParticipantDocument: &participantDoc,
		RelatedPartyDocument:                &relatedPartyDoc,
	}
...
func TestMongoDBRepository_appendEncryptedFilters_ErrorPropagation(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			query: http.QueryHeader{
				RegulatoryFieldsParticipantDocument: testutils.Ptr("98765432109876"),
			},
		...
		},
		{
			...
			query: http.QueryHeader{
				RelatedPartyDocument: testutils.Ptr("55566677788"),
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := repo.buildAliasFilter(ctx, organizationID, tt.query, holderID, false)
...
func (f *failingSearchTokenCandidatesEncryptor) GenerateSearchToken(_ context.Context, _ encryption.SearchTokenContext, _ string) (string, error) {
	return "token", nil
}
...
func TestMongoDBRepository_appendEncryptedFilters_MultiTokenKeyRotation_IndividualFields(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			query: http.QueryHeader{
				BankingDetailsAccount: testutils.Ptr("account-value"),
			},
		...
		},
		{
			...
			query: http.QueryHeader{
				BankingDetailsIban: testutils.Ptr("iban-value"),
			},
		...
		},
		{
			...
			query: http.QueryHeader{
				RegulatoryFieldsParticipantDocument: testutils.Ptr("participant-doc"),
			},
		...
		},
		{
			...
			query: http.QueryHeader{
				RelatedPartyDocument: testutils.Ptr("related-party-doc"),
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			filter, err := repo.buildAliasFilter(ctx, organizationID, tt.query, holderID, true)
			require.NoError(t, err)
...
func TestMongoDBRepository_appendEncryptedFilters_MultiTokenKeyRotation_OrderPreserved(t *testing.T) {
	...
	filter, err := repo.buildAliasFilter(ctx, organizationID, query, uuid.Nil, true)
	require.NoError(t, err)
