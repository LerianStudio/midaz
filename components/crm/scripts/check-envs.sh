...
echo ""
echo "${CYAN}Checking for .env files in workflows...${NC}"

MISSING_ENV_FILES=false

# Check if .env exists
if [ -f ".env" ]; then
    echo "${GREEN}${BOLD}[OK]${NC} workflows has .env file"
else
    # Check if .env.example exists
    if [ -f ".env.example" ]; then
        echo "${YELLOW}${BOLD}[MISSING]${NC} workflows is missing .env file (but has .env.example)"
        MISSING_ENV_FILES=true
    else
        echo "${CYAN}${BOLD}[INFO]${NC} workflows does not have .env or .env.example files"
    fi
fi
