...
func brokenRepo() *LedgerPostgreSQLRepository {
...
func TestLedgerRepository_GetDBFailures(t *testing.T) {
	t.Parallel()
	...
	t.Run("Create", func(t *testing.T) {
		...
		_, err := brokenRepo().Create(ctx, &mmodel.Ledger{})
		require.Error(t, err)
	})
	t.Run("Find", func(t *testing.T) {
		t.Parallel()

		_, err := brokenRepo().Find(ctx, uuid.New(), uuid.New())
		require.Error(t, err)
	})
	t.Run("FindAll", func(t *testing.T) {
		t.Parallel()

		_, err := brokenRepo().FindAll(ctx, uuid.New(), http.Pagination{Limit: 1, Page: 1, SortOrder: "asc"})
		require.Error(t, err)
	})
	t.Run("FindByName", func(t *testing.T) {
		t.Parallel()

		_, err := brokenRepo().FindByName(ctx, uuid.New(), "n")
		require.Error(t, err)
	})
	t.Run("ListByIDs", func(t *testing.T) {
		t.Parallel()

		_, err := brokenRepo().ListByIDs(ctx, uuid.New(), []uuid.UUID{uuid.New()})
		require.Error(t, err)
	})
	t.Run("Update", func(t *testing.T) {
		t.Parallel()

		_, err := brokenRepo().Update(ctx, uuid.New(), uuid.New(), &mmodel.Ledger{Name: "x"})
		require.Error(t, err)
	})
	t.Run("Delete", func(t *testing.T) {
		t.Parallel()

		err := brokenRepo().Delete(ctx, uuid.New(), uuid.New())
		require.Error(t, err)
	})
	t.Run("Count", func(t *testing.T) {
		t.Parallel()

		_, err := brokenRepo().Count(ctx, uuid.New())
		require.Error(t, err)
	})
}
