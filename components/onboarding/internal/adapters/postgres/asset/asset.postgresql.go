...
func NewAssetPostgreSQLRepository(pc *libPostgres.PostgresConnection) (*AssetPostgreSQLRepository, error) {
...
func (r *AssetPostgreSQLRepository) Create(ctx context.Context, asset *mmodel.Asset) (*mmodel.Asset, error) {
	...
	if rowsAffected == 0 {
		...
		return nil, fmt.Errorf("create asset no rows affected: %w", err)
	}
...
func (r *AssetPostgreSQLRepository) FindByNameOrCode(ctx context.Context, organizationID, ledgerID uuid.UUID, name, code string) (bool, error) {
	...
	if err != nil {
		...
		return false, fmt.Errorf("build find by name or code query: %w", err)
	}
	...
	if err != nil {
		...
		return false, fmt.Errorf("execute find by name or code query: %w", err)
	}
	defer rows.Close()
	...
	if rows.Next() {
		...
		return true, fmt.Errorf("asset name or code duplicate: %w", err)
	}

	if err := rows.Err(); err != nil {
		libOpentelemetry.HandleSpanError(&span, "Failed to iterate rows", err)

		return false, fmt.Errorf("iterate find by name or code rows: %w", err)
	}
...
func (r *AssetPostgreSQLRepository) Find(ctx context.Context, organizationID, ledgerID, id uuid.UUID) (*mmodel.Asset, error) {
	...
	if err := row.Scan(&asset.ID, &asset.Name, &asset.Type, &asset.Code, &asset.Status, &asset.StatusDescription,
		...
		if errors.Is(err, sql.ErrNoRows) {
			return nil, fmt.Errorf("asset not found: %w",
				pkg.ValidateBusinessError(constant.ErrEntityNotFound, reflect.TypeOf(mmodel.Asset{}).Name()))
		}
...
func (r *AssetPostgreSQLRepository) Update(ctx context.Context, organizationID, ledgerID, id uuid.UUID, asset *mmodel.Asset) (*mmodel.Asset, error) {
	...
	if rowsAffected == 0 {
		...
		return nil, fmt.Errorf("update asset no rows affected: %w", err)
	}
...
func (r *AssetPostgreSQLRepository) Delete(ctx context.Context, organizationID, ledgerID, id uuid.UUID) error {
	...
	if rowsAffected == 0 {
		...
		return fmt.Errorf("delete asset no rows affected: %w", err)
	}
