...
func brokenRepo() *PortfolioPostgreSQLRepository {
...
func TestPortfolioRepository_GetDBFailures(t *testing.T) {
	t.Parallel()
	...
	t.Run("Create", func(t *testing.T) {
		...
		_, err := brokenRepo().Create(ctx, &mmodel.Portfolio{})
		require.Error(t, err)
	})
	t.Run("FindByIDEntity", func(t *testing.T) {
		t.Parallel()

		_, err := brokenRepo().FindByIDEntity(ctx, uuid.New(), uuid.New(), uuid.New())
		require.Error(t, err)
	})
	...
	t.Run("Update", func(t *testing.T) {
		...
		_, err := brokenRepo().Update(ctx, uuid.New(), uuid.New(), uuid.New(), &mmodel.Portfolio{Name: "x"})
		require.Error(t, err)
	})
