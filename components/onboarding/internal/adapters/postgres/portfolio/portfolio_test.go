...
func TestPortfolioPostgreSQLModel_FromEntity(t *testing.T) {
	t.Run("with_all_fields_populated", func(t *testing.T) {
		statusDesc := "Active portfolio"
		deletedAt := time.Now().Add(-24 * time.Hour)

		entity := &mmodel.Portfolio{
		...
		var model PortfolioPostgreSQLModel
	...
	t.Run("generates_uuid_v7", func(t *testing.T) {
		...
		var model1 PortfolioPostgreSQLModel
		var model2 PortfolioPostgreSQLModel
