...
// Chaos tests for getDB in the organization PostgreSQL repository.
//
...
// Run with:
//
//	CHAOS=1 go test -tags integration -v -run TestIntegration_Chaos_Organization ./components/onboarding/internal/adapters/postgres/organization/...
...
// chaosNetworkTestInfra holds all resources for organization chaos tests with
...
// setupOrganizationChaosNetworkInfra creates the full chaos test infrastructure:
...
//  5. Builds an OrganizationPostgreSQLRepository connected through the proxy
//
// All cleanup (proxy deletion, container termination, network removal) is
// registered with t.Cleanup() automatically.
func setupOrganizationChaosNetworkInfra(t *testing.T) *chaosNetworkTestInfra {
	t.Helper()
...
// createTestOrganization creates an organization via the repository for chaos testing.
func (infra *chaosNetworkTestInfra) createTestOrganization(t *testing.T, name string) *mmodel.Organization {
	t.Helper()

	org := &mmodel.Organization{
		LegalName:     name,
		LegalDocument: "99999999000100",
		Status:        mmodel.Status{Code: "ACTIVE"},
		CreatedAt:     time.Now(),
		UpdatedAt:     time.Now(),
	}

	created, err := infra.repo.Create(context.Background(), org)
	require.NoError(t, err, "chaos infra: failed to create test organization")
...
func TestIntegration_Chaos_Organization_ConnectionLoss(t *testing.T) {
	...
	infra := setupOrganizationChaosNetworkInfra(t)
	...
	org := infra.createTestOrganization(t, "Chaos Connection Loss Org")
	require.NotEmpty(t, org.ID, "Phase 1: organization must have an ID")

	found, err := infra.repo.Find(ctx, parseOrgID(t, org.ID))
	require.NoError(t, err, "Phase 1: Find should succeed before fault injection")
	assert.Equal(t, org.LegalName, found.LegalName, "Phase 1: data should match")

	t.Logf("Phase 1: created organization %s", org.ID)
	...
	require.NotPanics(t, func() {
		...
		_, findErr = infra.repo.Find(findCtx, parseOrgID(t, org.ID))
	...
	require.NotPanics(t, func() {
		...
		createOrg := &mmodel.Organization{
			LegalName:     "Should Fail Org",
			LegalDocument: "00000000000001",
			Status:        mmodel.Status{Code: "ACTIVE"},
			CreatedAt:     time.Now(),
			UpdatedAt:     time.Now(),
		}

		_, createErr = infra.repo.Create(createCtx, createOrg)
	...
	require.NotPanics(t, func() {
		...
		_, countErr = infra.repo.Count(countCtx)
	}, "Phase 3: Count must not panic on connection loss")

	if countErr != nil {
		t.Logf("Phase 3: Count returned expected error: %v", countErr)
	} else {
		t.Log("Phase 3: Count succeeded (pool had cached connections)")
	}
	...
	chaos.AssertRecoveryWithin(t, func() error {
		_, err := infra.repo.Find(ctx, parseOrgID(t, org.ID))
		return err
	}, 30*time.Second, "Phase 5: Find should recover after proxy restoration")

	recovered, err := infra.repo.Find(ctx, parseOrgID(t, org.ID))
...
func TestIntegration_Chaos_Organization_HighLatency(t *testing.T) {
	if os.Getenv("CHAOS") != "1" {
		t.Skip("Set CHAOS=1 to run chaos tests")
	}
	...
	org := infra.createTestOrganization(t, "Chaos High Latency Org")
	...
	require.NotPanics(t, func() {
		...
		_, timeoutErr = infra.repo.Find(shortCtx, parseOrgID(t, org.ID))
	...
	slowFound, slowErr := infra.repo.Find(longCtx, parseOrgID(t, org.ID))
	slowElapsed := time.Since(start)

	if slowErr == nil {
		assert.Equal(t, org.ID, slowFound.ID, "Phase 3: data should be correct despite latency")
	...
	chaos.AssertRecoveryWithin(t, func() error {
		...
		_, err := infra.repo.Find(recoveryCtx, parseOrgID(t, org.ID))
		return err
...
func TestIntegration_Chaos_Organization_NetworkPartition(t *testing.T) {
	if os.Getenv("CHAOS") != "1" {
		t.Skip("Set CHAOS=1 to run chaos tests")
	}
	...
	staticOrg := infra.createTestOrganization(t, "Chaos Partition Static Org")
	require.NotEmpty(t, staticOrg.ID, "Phase 1: static org must have an ID")

	staticFound, err := infra.repo.Find(ctx, parseOrgID(t, staticOrg.ID))
	require.NoError(t, err, "Phase 1: static Find should succeed")
	assert.Equal(t, staticOrg.LegalName, staticFound.LegalName)

	// 1b. Tenant path (tenant DB injected into context).
	tenantFound, err := infra.repo.Find(tenantCtx, parseOrgID(t, staticOrg.ID))
	require.NoError(t, err, "Phase 1: tenant Find should succeed")
	assert.Equal(t, staticOrg.LegalName, tenantFound.LegalName)

	t.Logf("Phase 1: organization %s accessible via both static and tenant paths", staticOrg.ID)
	...
	require.NotPanics(t, func() {
		...
		_, staticPartitionErr = infra.repo.Find(partitionCtx, parseOrgID(t, staticOrg.ID))
	...
	require.NotPanics(t, func() {
		...
		deletePartitionErr = infra.repo.Delete(deleteCtx, parseOrgID(t, staticOrg.ID))
	...
	chaos.AssertRecoveryWithin(t, func() error {
		_, err := infra.repo.Find(ctx, parseOrgID(t, staticOrg.ID))
		return err
	}, 30*time.Second, "Phase 5: static Find should recover after partition heals")

	// 5b. Tenant path recovery.
	chaos.AssertRecoveryWithin(t, func() error {
		_, err := infra.repo.Find(tenantCtx, parseOrgID(t, staticOrg.ID))
		return err
	}, 30*time.Second, "Phase 5: tenant Find should recover after partition heals")

	// 5c. Data integrity verification.
	staticRecovered, err := infra.repo.Find(ctx, parseOrgID(t, staticOrg.ID))
	require.NoError(t, err, "Phase 5: static Find must succeed")

	// If the delete during partition succeeded, the org may be soft-deleted.
	// Otherwise, it should still be present with the original data.
	if deletePartitionErr != nil {
		// Delete failed during partition, so org should be intact.
		assert.Equal(t, staticOrg.ID, staticRecovered.ID,
			"Phase 5: organization ID must be unchanged after partition recovery")
		assert.Equal(t, staticOrg.LegalName, staticRecovered.LegalName,
...
// parseOrgID parses a UUID string from an organization ID for use in
// repository methods that require uuid.UUID.
func parseOrgID(t *testing.T, id string) uuid.UUID {
	t.Helper()

	parsed, err := uuid.Parse(id)
	require.NoError(t, err, "failed to parse organization ID %q as UUID", id)
