...
func brokenRepo() *OrganizationPostgreSQLRepository {
...
func TestOrgRepository_GetDBFailures(t *testing.T) {
	t.Parallel()
	...
	t.Run("Create", func(t *testing.T) {
		...
		_, err := brokenRepo().Create(ctx, validOrg())
		require.Error(t, err)
	})
	t.Run("Update", func(t *testing.T) {
		t.Parallel()

		_, err := brokenRepo().Update(ctx, uuid.New(), &mmodel.Organization{LegalName: "x"})
		require.Error(t, err)
	})
	t.Run("Find", func(t *testing.T) {
		t.Parallel()

		_, err := brokenRepo().Find(ctx, uuid.New())
		require.Error(t, err)
	})
	t.Run("FindAll", func(t *testing.T) {
		t.Parallel()

		_, err := brokenRepo().FindAll(ctx, http.Pagination{Limit: 1, Page: 1, SortOrder: "asc"})
		require.Error(t, err)
	})
	t.Run("ListByIDs", func(t *testing.T) {
		t.Parallel()

		_, err := brokenRepo().ListByIDs(ctx, []uuid.UUID{uuid.New()})
		require.Error(t, err)
	})
	t.Run("Delete", func(t *testing.T) {
		t.Parallel()

		err := brokenRepo().Delete(ctx, uuid.New())
		require.Error(t, err)
	})
	t.Run("Count", func(t *testing.T) {
		t.Parallel()

		_, err := brokenRepo().Count(ctx)
		require.Error(t, err)
	})
}
