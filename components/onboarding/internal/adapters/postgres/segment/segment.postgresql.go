...
func NewSegmentPostgreSQLRepository(pc *libPostgres.PostgresConnection) (*SegmentPostgreSQLRepository, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to connect to segment database: %w", err)
	}
...
func (p *SegmentPostgreSQLRepository) Create(ctx context.Context, segment *mmodel.Segment) (*mmodel.Segment, error) {
	...
	if rowsAffected == 0 {
		...
		return nil, fmt.Errorf("create segment no rows affected: %w", err)
	}
...
func (p *SegmentPostgreSQLRepository) FindByName(ctx context.Context, organizationID, ledgerID uuid.UUID, name string) (bool, error) {
	...
	if err != nil {
		...
		return false, fmt.Errorf("build find by name query: %w", err)
	}
	...
	if err != nil {
		...
		return false, fmt.Errorf("execute find by name query: %w", err)
	}
	defer rows.Close()
	...
	if rows.Next() {
		...
		return true, fmt.Errorf("duplicate segment name: %w", err)
	}

	if err := rows.Err(); err != nil {
		libOpentelemetry.HandleSpanError(&span, "Failed to iterate rows", err)

		return false, fmt.Errorf("iterate find by name rows: %w", err)
	}
...
func (p *SegmentPostgreSQLRepository) FindAll(ctx context.Context, organizationID, ledgerID uuid.UUID, filter http.Pagination) ([]*mmodel.Segment, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("execute find all query: %w",
			pkg.ValidateBusinessError(constant.ErrEntityNotFound, reflect.TypeOf(mmodel.Segment{}).Name()))
	}
	defer rows.Close()
...
func (p *SegmentPostgreSQLRepository) FindByIDs(ctx context.Context, organizationID, ledgerID uuid.UUID, ids []uuid.UUID) ([]*mmodel.Segment, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("build find by ids query: %w", err)
	}
	...
	if err != nil {
		...
		return nil, fmt.Errorf("execute find by ids query: %w", err)
	}
	defer rows.Close()
...
func (p *SegmentPostgreSQLRepository) Update(ctx context.Context, organizationID, ledgerID, id uuid.UUID, prd *mmodel.Segment) (*mmodel.Segment, error) {
	...
	if rowsAffected == 0 {
		...
		return nil, fmt.Errorf("update segment no rows affected: %w", err)
	}
...
func (p *SegmentPostgreSQLRepository) Delete(ctx context.Context, organizationID, ledgerID, id uuid.UUID) error {
	...
	if rowsAffected == 0 {
		...
		return fmt.Errorf("delete segment no rows affected: %w", err)
	}
