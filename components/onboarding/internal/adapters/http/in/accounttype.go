...
// AccountTypeHandler struct contains an account type use case for managing account type related operations.
type AccountTypeHandler struct {
	Command *command.UseCase
	Query   *query.UseCase
}

// CreateAccountType creates an Account Type.
//
...
func (handler *AccountTypeHandler) CreateAccountType(i any, c *fiber.Ctx) error {
	...
	payload, ok := i.(*mmodel.CreateAccountTypeInput)
	if !ok {
...
func (handler *AccountTypeHandler) GetAccountTypeByID(c *fiber.Ctx) error {
	...
	if err != nil {
		...
		logger.Errorf("Failed to retrieve Account Type with ID: %s, Error: %s", id.String(), err)
...
func (handler *AccountTypeHandler) UpdateAccountType(i any, c *fiber.Ctx) error {
	...
	payload, ok := i.(*mmodel.UpdateAccountTypeInput)
	if !ok {
		return http.WithError(c, fiber.NewError(fiber.StatusBadRequest, "invalid payload type"))
	}
	...
	if err != nil {
		...
		logger.Errorf("Failed to update account type with ID: %s, Error: %s", id.String(), err)
	...
	if err != nil {
		...
		logger.Errorf("Failed to get updated account type with ID: %s, Error: %s", id.String(), err)
...
func (handler *AccountTypeHandler) GetAllAccountTypes(c *fiber.Ctx) error {
	...
	if headerParams.Metadata != nil {
		...
		if err != nil {
			...
			logger.Errorf("Failed to retrieve all Account Types, Error: %s", err)
	...
	if err != nil {
		...
		logger.Errorf("Failed to retrieve Account Types, Error: %s", err)
