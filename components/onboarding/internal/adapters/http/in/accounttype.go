...
func (handler *AccountTypeHandler) CreateAccountType(i any, c *fiber.Ctx) error {
	...
	logger.Log(ctx, libLog.LevelInfo, fmt.Sprintf("Request to create an account type with details: %#v", payload))
...
func (handler *AccountTypeHandler) UpdateAccountType(i any, c *fiber.Ctx) error {
	...
	logger.Log(ctx, libLog.LevelInfo, fmt.Sprintf("Request to update account type with ID: %s and details: %#v", id, payload))
	...
	logger.Log(ctx, libLog.LevelInfo, fmt.Sprintf("Successfully updated account type with ID: %s", id))
...
//	@Success		200				{object}	http.Pagination{items=[]mmodel.AccountType,next_cursor=string,prev_cursor=string,limit=int,page=int}	"Successfully retrieved account types"
