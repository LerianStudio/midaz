...
// errGRPCConnectionFailed is a sentinel error for testing gRPC connection failures.
var errGRPCConnectionFailed = errors.New("gRPC connection failed")

// errGRPCDeleteFailed is a sentinel error for testing gRPC delete operation failures.
var errGRPCDeleteFailed = errors.New("gRPC delete operation failed")
...
func (a *testableBalanceAdapter) createBalanceSync(ctx context.Context, input mmodel.CreateBalanceInput) (*mmodel.Balance, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create balance via gRPC: %w", err)
	}
...
func (a *testableBalanceAdapter) deleteAllBalancesByAccountID(ctx context.Context, organizationID, ledgerID, accountID uuid.UUID, requestID string) error {
	...
	if err := a.repo.DeleteAllBalancesByAccountID(ctx, token, req); err != nil {
		return fmt.Errorf("delete all balances by account ID via gRPC: %w", err)
	}

	return nil
}

// createBalanceSyncTestCase defines a test case for createBalanceSync.
type createBalanceSyncTestCase struct {
	name        string
...
// runCreateBalanceSyncTests is a shared runner for createBalanceSync table tests.
func runCreateBalanceSyncTests(t *testing.T, tests []createBalanceSyncTestCase) {
	t.Helper()
...
// buildSuccessTestCases returns test cases for successful createBalanceSync scenarios.
func buildSuccessTestCases(orgID, ledgerID, accountID uuid.UUID) []createBalanceSyncTestCase {
	return []createBalanceSyncTestCase{
		{
			name: "success",
			input: mmodel.CreateBalanceInput{
				RequestID: "req-123", OrganizationID: orgID, LedgerID: ledgerID, AccountID: accountID,
				Alias: "@user1", Key: "default", AssetCode: "USD", AccountType: "deposit",
				...
				mockRepo.EXPECT().
					...
					DoAndReturn(func(_ context.Context, _ string, req *proto.BalanceRequest) (*proto.BalanceResponse, error) {
						...
						return &proto.BalanceResponse{
							Id: "balance-id-123", Alias: "@user1", Key: "default", AssetCode: "USD",
							Available: "1000.50", OnHold: "100.25", AllowSending: true, AllowReceiving: true,
						}, nil
					})
			},
			validate: func(t *testing.T, result *mmodel.Balance) {
		...
		},
		{
			name: "zero_balances",
			input: mmodel.CreateBalanceInput{
				RequestID: "req-zero", OrganizationID: orgID, LedgerID: ledgerID, AccountID: accountID,
				Alias: "@zero-user", Key: "default", AssetCode: "JPY", AccountType: "deposit",
				AllowSending: true, AllowReceiving: true,
			},
			setupMocks: func(t *testing.T, mockRepo *MockRepository) {
				t.Helper()
				mockRepo.EXPECT().CreateBalance(gomock.Any(), gomock.Any(), gomock.Any()).
					Return(&proto.BalanceResponse{
						Id: "balance-zero", Alias: "@zero-user", Key: "default", AssetCode: "JPY",
						Available: "0", OnHold: "0", AllowSending: true, AllowReceiving: true,
					}, nil)
			},
			validate: func(t *testing.T, result *mmodel.Balance) {
				t.Helper()
		...
		},
		{
			name: "high_precision",
			input: mmodel.CreateBalanceInput{
				RequestID: "req-highprec", OrganizationID: orgID, LedgerID: ledgerID, AccountID: accountID,
				Alias: "@highprec-user", Key: "default", AssetCode: "BTC", AccountType: "deposit",
				AllowSending: true, AllowReceiving: true,
			},
			setupMocks: func(t *testing.T, mockRepo *MockRepository) {
				t.Helper()
				mockRepo.EXPECT().CreateBalance(gomock.Any(), gomock.Any(), gomock.Any()).
					Return(&proto.BalanceResponse{
						Id: "balance-highprec", Alias: "@highprec-user", Key: "default", AssetCode: "BTC",
						Available: "999999999999.123456789", OnHold: "0.000000001", AllowSending: true, AllowReceiving: true,
					}, nil)
			},
			validate: func(t *testing.T, result *mmodel.Balance) {
				t.Helper()
...
func TestBalanceAdapter_CreateBalanceSync_Success(t *testing.T) {
	t.Parallel()

	runCreateBalanceSyncTests(t, buildSuccessTestCases(uuid.New(), uuid.New(), uuid.New()))
}

// buildErrorTestCases returns test cases for error scenarios in createBalanceSync.
func buildErrorTestCases(orgID, ledgerID, accountID uuid.UUID) []createBalanceSyncTestCase {
	return []createBalanceSyncTestCase{
		{
			name: "grpc_error",
			input: mmodel.CreateBalanceInput{
				RequestID: "req-456", OrganizationID: orgID, LedgerID: ledgerID, AccountID: accountID,
				Alias: "@user2", Key: "default", AssetCode: "BRL", AccountType: "savings",
				AllowSending: false, AllowReceiving: true,
			},
			setupMocks: func(t *testing.T, mockRepo *MockRepository) {
				t.Helper()
				mockRepo.EXPECT().CreateBalance(gomock.Any(), gomock.Any(), gomock.Any()).
					Return(nil, errGRPCConnectionFailed)
			},
			wantErr: true, errContains: "gRPC connection failed",
		},
		{
			name: "available_decimal_parse_error",
			input: mmodel.CreateBalanceInput{
				RequestID: "req-789", OrganizationID: orgID, LedgerID: ledgerID, AccountID: accountID,
				Alias: "@user3", Key: "default", AssetCode: "EUR", AccountType: "checking",
				AllowSending: true, AllowReceiving: false,
			},
			setupMocks: func(t *testing.T, mockRepo *MockRepository) {
				t.Helper()
				mockRepo.EXPECT().CreateBalance(gomock.Any(), gomock.Any(), gomock.Any()).
					Return(&proto.BalanceResponse{
						Id: "balance-id-456", Alias: "@user3", Key: "default", AssetCode: "EUR",
						Available: "not-a-number", OnHold: "50.00", AllowSending: true, AllowReceiving: false,
					}, nil)
			},
			wantErr: true, errContains: "failed to parse Available for balance balance-id-456",
		},
		{
			name: "onhold_decimal_parse_error",
			input: mmodel.CreateBalanceInput{
				RequestID: "req-101", OrganizationID: orgID, LedgerID: ledgerID, AccountID: accountID,
				Alias: "@user4", Key: "freeze", AssetCode: "GBP", AccountType: "external",
				AllowSending: false, AllowReceiving: false,
			},
			setupMocks: func(t *testing.T, mockRepo *MockRepository) {
				t.Helper()
				mockRepo.EXPECT().CreateBalance(gomock.Any(), gomock.Any(), gomock.Any()).
					Return(&proto.BalanceResponse{
						Id: "balance-id-789", Alias: "@user4", Key: "freeze", AssetCode: "GBP",
						Available: "200.00", OnHold: "invalid-decimal", AllowSending: false, AllowReceiving: false,
					}, nil)
			},
			wantErr: true, errContains: "failed to parse OnHold for balance balance-id-789",
		},
		{
			name: "empty_decimal",
			input: mmodel.CreateBalanceInput{
				RequestID: "req-empty", OrganizationID: orgID, LedgerID: ledgerID, AccountID: accountID,
				Alias: "@empty-user", Key: "default", AssetCode: "EUR", AccountType: "deposit",
				AllowSending: true, AllowReceiving: true,
			},
			setupMocks: func(t *testing.T, mockRepo *MockRepository) {
				t.Helper()
				mockRepo.EXPECT().CreateBalance(gomock.Any(), gomock.Any(), gomock.Any()).
					Return(&proto.BalanceResponse{
						Id: "balance-empty-decimal", Alias: "@empty-user", Key: "default", AssetCode: "EUR",
						Available: "", OnHold: "0", AllowSending: true, AllowReceiving: true,
					}, nil)
			},
			wantErr: true, errContains: "failed to parse Available for balance balance-empty-decimal",
		},
	}
}

func TestBalanceAdapter_CreateBalanceSync_Errors(t *testing.T) {
	t.Parallel()

	runCreateBalanceSyncTests(t, buildErrorTestCases(uuid.New(), uuid.New(), uuid.New()))
}
