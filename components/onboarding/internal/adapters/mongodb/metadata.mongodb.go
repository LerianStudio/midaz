...
func NewMetadataMongoDBRepository(mc *libMongo.MongoConnection) (*MetadataMongoDBRepository, error) {
...
func (mmr *MetadataMongoDBRepository) FindList(ctx context.Context, collection string, filter http.QueryHeader) ([]*Metadata, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("find metadata: %w", err)
	}
	...
	if err := cur.Err(); err != nil {
		...
		return nil, fmt.Errorf("iterate metadata cursor: %w", err)
	}
	...
	if err := cur.Close(ctx); err != nil {
		...
		return nil, fmt.Errorf("close metadata cursor: %w", err)
	}
...
func (mmr *MetadataMongoDBRepository) FindByEntity(ctx context.Context, collection, id string) (*Metadata, error) {
	...
	if err = coll.FindOne(ctx, bson.M{"entity_id": id}).Decode(&record); err != nil {
		...
		return nil, fmt.Errorf("find metadata by entity: %w", err)
	}
...
func (mmr *MetadataMongoDBRepository) FindByEntityIDs(ctx context.Context, collection string, entityIDs []string) ([]*Metadata, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("find metadata by entity IDs: %w", err)
	}
	defer cur.Close(ctx)
...
func (mmr *MetadataMongoDBRepository) Update(ctx context.Context, collection, id string, metadata map[string]any) error {
	...
	if err != nil {
		if errors.Is(err, mongo.ErrNoDocuments) {
			businessErr := pkg.ValidateBusinessError(constant.ErrEntityNotFound, collection)

			libOpentelemetry.HandleSpanBusinessErrorEvent(&spanUpdate, "Failed to update metadata", businessErr)

			logger.Warnf("Failed to update metadata: %v", businessErr)

			return fmt.Errorf("entity not found for update: %w", businessErr)
		}
...
func (mmr *MetadataMongoDBRepository) CreateIndex(ctx context.Context, collection string, input *mmodel.CreateMetadataIndexInput) (*mmodel.MetadataIndex, error) {
	...
	indexName := "metadata." + input.MetadataKey
...
func (mmr *MetadataMongoDBRepository) FindAllIndexes(ctx context.Context, collection string) ([]*mmodel.MetadataIndex, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("aggregate index stats: %w", err)
	}
	...
	if err := cur.Err(); err != nil {
		...
		return nil, fmt.Errorf("iterate metadata index cursor: %w", err)
	}
...
func (mmr *MetadataMongoDBRepository) DeleteIndex(ctx context.Context, collection, indexName string) error {
	...
	if err != nil {
		...
		if errors.As(err, &cmdErr) && cmdErr.Name == "IndexNotFound" {
			return fmt.Errorf("index not found: %w", pkg.ValidateBusinessError(constant.ErrMetadataIndexNotFound, "metadata_index"))
		}
