	...
	}{
		{
			...
			setupMocks: func() {
				...
				mockAccountRepo.EXPECT().
					FindAll(gomock.Any(), organizationID, ledgerID, &portfolioID, (*uuid.UUID)(nil), filter.ToOffsetPagination()).
...
func TestGetAllAccount_WithSegmentID(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	tests := []struct {
		...
		{
			name:        "success - accounts filtered by segment_id",
			portfolioID: nil,
			segmentID:   &segmentID,
			setupMocks: func() {
				bFalse := false
				mockAccountRepo.EXPECT().
					FindAll(gomock.Any(), organizationID, ledgerID, (*uuid.UUID)(nil), &segmentID, filter.ToOffsetPagination()).
					Return([]*mmodel.Account{
						{ID: "acc1", Blocked: &bFalse},
					}, nil).
					Times(1)
		...
		{
			name:        "success - no segment_id filter applied (backward compat)",
			portfolioID: nil,
			segmentID:   nil,
			setupMocks: func() {
				bFalse := false
				mockAccountRepo.EXPECT().
					FindAll(gomock.Any(), organizationID, ledgerID, (*uuid.UUID)(nil), (*uuid.UUID)(nil), filter.ToOffsetPagination()).
					Return([]*mmodel.Account{
						{ID: "acc1", Blocked: &bFalse},
						{ID: "acc2", Blocked: &bFalse},
					}, nil).
					Times(1)
			...
			expectedAccounts: []*mmodel.Account{
				func() *mmodel.Account {
					b := false
					return &mmodel.Account{ID: "acc1", Blocked: &b}
				}(),
				func() *mmodel.Account {
					b := false
					return &mmodel.Account{ID: "acc2", Blocked: &b}
				}(),
			},
		},
	}
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedErr != nil {
				...
				for i, acc := range result {
					assert.Equal(t, tt.expectedAccounts[i].ID, acc.ID)
				}
			}
		})
	}
