...
# Specs merged into the unified MIDAZ collection, as
# "component|spec-file|version-tag|requirement".
#
# The version tag disambiguates when one component publishes more than one spec: it keeps
# per-spec temp files apart and is appended to folder names, so two same-surface specs from
# one component stay distinct folders after the merge. It is empty for a component that
# publishes a single spec.
#
...
for source in "${SPEC_SOURCES[@]}"; do
    IFS='|' read -r component spec_file version requirement <<< "${source}"
