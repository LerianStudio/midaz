...
# Process ledger component result
...
if [ "$LEDGER_STATUS" = "SUCCESS" ]; then
    echo "Processing ledger component collection..."
    ...
    echo -e "${RED}Ledger OpenAPI spec was not successfully converted.${NC}"
    rm -rf "${TEMP_DIR}"
    exit 1
fi
