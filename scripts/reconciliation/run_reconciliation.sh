...
WITH balance_calc AS (
    SELECT
        ...
        -- ON_HOLD subtracts from available only while transaction is still PENDING
        ...
        COALESCE(SUM(CASE WHEN o.type = 'ON_HOLD' AND o.balance_affected = true AND t.status = 'PENDING' THEN o.amount ELSE 0 END), 0) as expected
    FROM balance b
