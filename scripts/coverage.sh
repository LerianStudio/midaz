#!/bin/bash
#
# This script generates a test coverage report for all Go packages in the project,
# excluding any packages listed in `coverage_ignore.txt`. It prints a summary to
# the console and creates an HTML report for detailed analysis.
...
echo "PostgreSQL repository tests are excluded from coverage metrics because they"
echo "use mock repositories that satisfy the Repository interface rather than"
echo "directly testing the actual implementation. This is a common pattern in Go"
echo "testing for database interactions, but it means the coverage report doesn't"
echo "accurately reflect the test coverage of the database code."
