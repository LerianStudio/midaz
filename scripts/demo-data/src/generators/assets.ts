...
export class AssetGenerator implements EntityGenerator<Asset> {
  ...
  async generate(count: number, parentId?: string): Promise<Asset[]> {
    ...
    for (let i = 0; i < Math.min(count, ASSET_TEMPLATES.length); i++) {
      ...
      try {
        ...
        this.logger.error(`Failed to generate template asset ${i + 1} for ledger ${ledgerId}`, error as Error);
  ...
  private async createAssetFromTemplate(
    ...
    try {
      ...
      // Create the asset
      const asset = await this.client.entities.assets.createAsset(
        organizationId,
        ledgerId,
        {
          code,
      ...
      if ((error as Error).message.includes('already exists') || 
        ...
        this.logger.warn(`Asset with code "${code}" may already exist for ledger ${ledgerId}, trying to retrieve it`);
