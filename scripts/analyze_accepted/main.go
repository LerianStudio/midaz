...
func fetchAliasAvailable(transURL, auth, org, ledger, alias, asset string) (float64, error) {
    ...
    if resp.StatusCode != 200 { io.ReadAll(resp.Body); return 0, fmt.Errorf("status %d", resp.StatusCode) }
    var body struct {
        Items []struct { AssetCode string `json:"assetCode"`; Available any `json:"available"` } `json:"items"`
    }
    ...
    for _, it := range body.Items {
        if it.AssetCode == asset {
            ...
            case float64:
                cur = v
            default:
                b, _ := json.Marshal(v); fmt.Sscan(string(b), &cur)
            }
        }
    }
    return cur, nil
...
func main() {
    ...
    type entry struct{ kind, id string; tx tx }
    var entries []entry
    ...
    for _, e := range entries {
        if strings.Contains(logStr, e.id) { found++ } else { missing++; if len(missIDs) < 50 { missIDs = append(missIDs, fmt.Sprintf("%s:%s", e.kind, e.id)) } }
    }
    ...
    for _, e := range entries {
        for _, op := range e.tx.Operations {
            if op.AccountAlias == "" || strings.HasPrefix(op.AccountAlias, "@external/") { continue }
            var after float64
            ...
            if v, ok := maxAfter[k]; !ok || after > v { maxAfter[k] = after }
        }
    }
    ...
    for k, exp := range maxAfter {
        ...
        if cur+1e-9 < exp { discrepancies = append(discrepancies, fmt.Sprintf("ALERT: alias=%s asset=%s current %.2f < expected_min %.2f", k.Alias, k.Asset, cur, exp)) }
    }
    ...
    if err := os.WriteFile(*outPath, []byte(report), 0644); err != nil { panic(err) }
}
