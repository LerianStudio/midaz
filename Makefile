...
help:
	...
	@echo ""
	@echo -e "$(BOLD)Git Hook Commands:$(NC)"
...
lint:
	@echo -e "$(BLUE)Running linter and performance checks...$(NC)"
	./make.sh "lint"
...
check-logs:
	@echo -e "$(BLUE)Checking error logging in usecases...$(NC)"
	./make.sh "checkLogs"
...
check-hooks:
	@echo -e "$(BLUE)Checking git hooks status...$(NC)"
	./make.sh "checkHooks"
...
set-env:
	...
	@echo -e "$(BLUE)Environment files created successfully$(NC)"
...
auth:
	@echo -e "$(BLUE)Executing command in auth service...$(NC)"
	$(MAKE) -C $(AUTH_DIR) $(COMMAND)

infra:
	@echo -e "$(BLUE)Executing command in infra service...$(NC)"
	$(MAKE) -C $(INFRA_DIR) $(COMMAND)

ledger:
	@echo -e "$(BLUE)Executing command in ledger service...$(NC)"
	$(MAKE) -C $(LEDGER_DIR) $(COMMAND)

transaction:
	@echo -e "$(BLUE)Executing command in transaction service...$(NC)"
	$(MAKE) -C $(TRANSACTION_DIR) $(COMMAND)

audit:
	@echo -e "$(BLUE)Executing command in audit service...$(NC)"
	$(MAKE) -C $(AUDIT_DIR) $(COMMAND)

all-services:
	@echo -e "$(BLUE)Executing command across all services...$(NC)"
	$(MAKE) -C $(AUTH_DIR) $(COMMAND) && \
	$(MAKE) -C $(INFRA_DIR) $(COMMAND) && \
	$(MAKE) -C $(LEDGER_DIR) $(COMMAND) && \
	$(MAKE) -C $(TRANSACTION_DIR) $(COMMAND)
...
goreleaser:
	@echo -e "$(BLUE)Creating release snapshot...$(NC)"
...
generate-docs-all:
	@echo -e "$(BLUE)Executing command to generate swagger...$(NC)"
