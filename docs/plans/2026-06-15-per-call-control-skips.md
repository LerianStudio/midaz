...
| 4 | **Audit + hardening (release gate):** honored skips persisted to typed columns + streaming event across the full wire path; OpenAPI/llms-full/docs; cross-surface integration suite | 4.1, 4.2, 4.3 | ⏳ Doing — 4a tx-audit ✅ (`c7caf2b2f`); 4b(acct) next → 4c(int+docs) |
...
**Status:** Doing — **4a (transaction half) DONE** (`c7caf2b2f`): migration 000035, domain+PG model fields, ToEntity/FromEntity, both column lists, single+bulk INSERT, all 6 scan sites (Scan order == SELECT order verified), handler wiring of resolved `honored*Skip`, `app.transaction.*` span attrs, full streaming wire path + JSONShape lock, column-count guards 16→18. **4b (account half) PENDING:** migration 000019, `mmodel.Account`+`AccountPostgreSQLModel` field, `accountColumnList`, hand-written `Create` INSERT literal, all 8 scan sites, use-case wiring of `honoredHolderSkip`, `AccountCreatedPayload`+`NewAccountCreated`+account JSONShape (count 16→17). JSONShape "exact-field-set" upgrade still deferred (kept present-key whitelist on both surfaces — flag for 4c if fail-closed is wanted).
