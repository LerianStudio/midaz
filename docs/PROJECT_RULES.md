...
The HTTP contract is generated by **Huma v2 (OAS 3.1)** over Fiber, and error responses follow
**RFC 9457** (`application/problem+json`) — a `problem.Detail` superset carrying `type`, `title`,
`status`, `detail`, `instance`, plus midaz's `code` and `entityType`.
...
swaggo `// @Summary` / `// @Router` doc comments. Each resource has a `*_handler_huma.go`
file under `components/ledger/internal/adapters/http/in/` that declares typed input/output
structs and registers operations with `huma.Register(api, huma.Operation{...})`:

```go
// CreateAccountInputHuma is the typed request envelope. Path params are plain strings
// (validated by the Fiber middleware chain, not by Huma); RawBody keeps the body out of
// Huma's validator so the existing decode/validate path owns 422s.
...
6. **Add HTTP handler:** Huma typed input/output in a `*_handler_huma.go` file
...
3. Create handler in `adapters/http/in/` (`*_handler_huma.go` with typed Huma input/output structs)
