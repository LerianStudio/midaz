...
|-----------|--------------|-----------|--------|
| `midaz` | ledger and CRM (collapsed package) — one `midazName` const | `organizations`, `ledgers`, `assets`, `asset-rates`, `portfolios`, `segments`, `accounts`, `balances`, `transactions`, `operations`, `settings`, `account-types`, `operation-routes`, `transaction-routes`, `holders`, `instruments` | `components/ledger/internal/adapters/http/in/routes.go` (`midazName = "midaz"`, helper `protectedMidaz`), which the per-resource `holder_routes.go` / `instrument_routes.go` / `encryption_routes.go` / `audit_routes.go` registrars call for the `holders`/`instruments` resources |
...
|-----------|-------------|----------------------|------------------------|
| `midaz` | ledger (`:3002`) | `organizations`, `ledgers`, `assets`, `asset-rates`, `portfolios`, `segments`, `accounts`, `balances`, `transactions`, `operations`, `settings`, `account-types`, `operation-routes`, `transaction-routes`, `holders`, `instruments` | `components/ledger/internal/adapters/http/in/routes.go` (`midazName`, helper `protectedMidaz`); `holder_routes.go` / `instrument_routes.go` for `holders`/`instruments` |
