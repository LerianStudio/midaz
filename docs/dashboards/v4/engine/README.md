...
`code`, recovery `source` (`legacy_backup` or `engine_recover`), and posting `type`
vocabularies. No panel groups by an identifier, alias, metadata, amount, or request payload.
