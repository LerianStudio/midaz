...
**Scope.** This standard governs error production, classification, mapping, and exposure across every Go service in the Midaz monorepo (module `github.com/LerianStudio/midaz/v4`): `components/ledger` (including the embedded CRM and fee packages) and `components/tracer`, plus the shared `pkg/` trees. (The reporter is no longer in this monorepo — it ships from its own standalone repo; the canonical reporter `file:line` examples below are preserved as evidence from the 2026-06-07 audit, when the reporter still lived here.) It is produced from the 2026-06-07 error-handling audit — ground truth in [`docs/plans/2026-06-07-telemetry-error-audit.json`](../plans/2026-06-07-telemetry-error-audit.json), with Phase 1 reconciliation in [`docs/plans/2026-06-07-telemetry-error-audit-appendix.md`](../plans/2026-06-07-telemetry-error-audit-appendix.md). It is the error-handling half of the normalization plan in [`docs/plans/2026-06-07-telemetry-error-normalization.md`](../plans/2026-06-07-telemetry-error-normalization.md); the telemetry half lives in [`docs/standards/telemetry.md`](./telemetry.md) (rules T1–T13).
