...
**Canonical example.** `components/crm/services/create-holder-with-id.go:73` — the `if mongo.IsDuplicateKeyError(err)` branch driving idempotent create: on a raw `_id` collision it re-fetches and returns the existing holder as success. (The repository wraps index collisions as typed business errors, so only the raw `_id` collision satisfies `IsDuplicateKeyError`.) A `strings.Contains(err.Error(), "...")` classification anywhere — e.g. the former reporter classifier `"TPL-"` text match — is the rejected shape; no such site remains in production code.
