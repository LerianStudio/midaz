...
export function createAccount(token, organizationId, ledgerId, assetCode, accountPrefix) {
  ...
  const payload = JSON.stringify({
    "assetCode": assetCode,
    "name": `Lerian ${assetCode} Account ${accountNumber}`,
...
export function chargeBalances(token, organizationId, ledgerId, accountAlias) {
  for (const balance of BALANCE_DATA) {
    ...
    const payload = JSON.stringify({
      "transactionDate": "2025-05-31T00:00:00Z",
      "send": {
      "asset": "BRL",
      "value": "100000",
      "source": {
...
export default async function (data) {
  const account = createAccount(data.token, data.organizationId, data.ledgerId, "BRL", "account:");
