...
func TestFuzz_Structural_OmittedUnknownInvalidJSONLarge(t *testing.T) {
    ...
    if c < 400 || c >= 500 { t.Fatalf("expected 4xx for omitted required fields; got %d body=%s", c, string(b)) }
    ...
    if c < 400 || c >= 500 { t.Fatalf("expected 4xx for unknown fields; got %d body=%s", c, string(b)) }
    ...
    if c < 400 || c >= 500 { t.Fatalf("expected 4xx for invalid json; got %d body=%s", c, string(b)) }
    ...
    if c >= 500 { t.Fatalf("server 5xx on large body: body=%s", string(b)) }
}
