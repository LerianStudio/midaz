// Package fuzzy provides fuzz testing for the Midaz API, focusing on input validation
// and unexpected edge cases that might not be covered by traditional unit tests.
package fuzzy
...
// FuzzCreateOrganizationName fuzzes the legalName field of the organization creation
// endpoint to validate input handling and prevent panics or 5xx errors.
//
// This test sends a wide range of mutated strings to the API to ensure that the
// server's validation logic is robust and can handle unexpected or malformed inputs
// gracefully.
//
