...
func TestFuzz_Accounts_AliasAndType(t *testing.T) {
	...
	for i := 0; i < 40; i++ {
		...
		if e != nil {
			t.Fatalf("account request error: %v", e)
		}
		if c >= 500 {
			t.Fatalf("server 5xx on fuzz account: %d body=%s (aliasLen=%d type=%s)", c, string(b), aliasLen, typ)
		}
...
func TestFuzz_Transactions_Amounts_And_Codes(t *testing.T) {
	...
	for i := 0; i < 40; i++ {
		...
		if !inflow {
			path = "/v1/organizations/%s/ledgers/%s/transactions/outflow"
		}
