...
func TestChaos_MongoDB_RestartGraceful(t *testing.T) {
    ...
    // Restart Mongo
    if err := h.RestartWithWait("midaz-mongodb", 5*time.Second); err != nil { t.Fatalf("restart mongodb: %v", err) }
    ...
    if err != nil || code != 200 { t.Fatalf("patch ledger metadata: %d %s", code, string(body)) }

    code, body, err = onboard.Request(ctx, "GET", fmt.Sprintf("/v1/organizations/%s/ledgers/%s", org.ID, ledger.ID), headers, nil)
    if err != nil || code != 200 { t.Fatalf("get ledger after mongo restart: %d %s", code, string(body)) }
    var got struct{ Metadata map[string]any `json:"metadata"` }
    _ = json.Unmarshal(body, &got)
    if got.Metadata["chaos"] != "mongo" { t.Fatalf("metadata not persisted after restart: %+v", got.Metadata) }
}
