...
// TestChaos_RestartDatabase simulates a database restart to verify the resilience
// and recovery capabilities of the Midaz services.
//
// This chaos experiment restarts the primary PostgreSQL database and then checks
// if the onboarding and transaction services can gracefully recover and resume
// normal operations.
func TestChaos_RestartDatabase(t *testing.T) {
	shouldRunChaos(t)
	// Capture logs for quick diagnostics
	defer h.StartLogCapture([]string{"midaz-transaction", "midaz-onboarding", "midaz-postgres-primary"}, "RestartDatabase")()
	...
	if err := h.WaitForHTTP200(env.OnboardingURL+"/health", 90*time.Second); err != nil {
		t.Fatalf("onboarding health after restart: %v", err)
	}
	if err := h.WaitForHTTP200(env.TransactionURL+"/health", 90*time.Second); err != nil {
