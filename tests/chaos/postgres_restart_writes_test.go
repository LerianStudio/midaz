...
// shouldRunChaos gates chaos tests; currently all chaos tests are allowed to run.
...
//
//nolint:gocognit,gocyclo,cyclop,funlen // chaos test deliberately exercises concurrent writes during restart; splitting would obscure the test scenario
func TestChaos_PostgresRestart_DuringWrites(t *testing.T) { //nolint:paralleltest // chaos tests interact with shared Docker infrastructure
	shouldRunChaos(t)
	// auto log capture for correlation
	cleanup := h.StartLogCapture([]string{"midaz-transaction", "midaz-onboarding", "midaz-postgres-primary"}, "PostgresRestart_DuringWrites")
	defer cleanup()
	...
	inflow := func(val string) {
		...
		for {
			...
			if c == 201 {
				...
				if m.ID != "" {
					accepted = append(accepted, acceptedRec{Kind: "inflow", ID: m.ID})
				}
				mu.Unlock()
			}
	...
	outflow := func(val string) {
		...
		for {
			...
			if c == 201 {
				...
				if m.ID != "" {
					accepted = append(accepted, acceptedRec{Kind: "outflow", ID: m.ID})
				}
				mu.Unlock()
			}
