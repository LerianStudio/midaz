...
// TestChaos_RedisRestart_ReadsRecover simulates a Redis restart to verify that the
// system can handle cache unavailability and recover gracefully.
//
// This chaos experiment restarts the Redis container and then continuously polls
// for account balances. It ensures that read operations, while potentially
// degraded during the outage, recover quickly and that data consistency is
// maintained.
func TestChaos_RedisRestart_ReadsRecover(t *testing.T) {
	...
	// Restart Redis
	if err := h.RestartWithWait("midaz-valkey", 4*time.Second); err != nil {
	...
	for {
		...
		if code == 200 {
			...
			if sum.IsNegative() {
				t.Fatalf("negative balance after redis restart: %s", sum.String())
			}
			recovered = true
			break
		}
	...
	if !recovered {
		t.Fatalf("balances did not recover after redis restart")
	}
}
