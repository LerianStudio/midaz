...
//
//nolint:gocognit,gocyclo,cyclop,funlen // chaos test deliberately exercises multiple account operations; splitting would obscure the test scenario
func TestChaos_PostChaosIntegrity_MultiAccount(t *testing.T) { //nolint:paralleltest // chaos tests interact with shared Docker infrastructure
	shouldRunChaos(t)
	// auto log capture for correlation
	cleanup := h.StartLogCapture([]string{"midaz-transaction", "midaz-onboarding", "midaz-postgres-primary"}, "PostChaosIntegrity_MultiAccount")
	defer cleanup()
