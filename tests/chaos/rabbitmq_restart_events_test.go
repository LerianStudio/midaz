...
// TestChaos_RabbitMQ_Restart_WithEvents simulates a RabbitMQ restart to verify
// that the system can handle the event broker's temporary unavailability without
// causing API failures.
//
// This chaos experiment restarts the RabbitMQ container and then posts a
// transaction, ensuring that the API continues to respond successfully (i.e., no
// 5xx errors) and that the system recovers gracefully.
func TestChaos_RabbitMQ_Restart_WithEvents(t *testing.T) {
	...
	// Restart RabbitMQ
	if err := h.RestartWithWait("midaz-rabbitmq", 6*time.Second); err != nil {
		t.Fatalf("restart rabbitmq: %v", err)
	}
	...
	if err != nil || code >= 500 {
		t.Fatalf("inflow after rmq restart: %d %s err=%v", code, string(body), err)
	}
}
