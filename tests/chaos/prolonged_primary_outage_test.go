...
// TestChaos_ProlongedPrimaryOutage_GracefulRecovery simulates a prolonged outage
// of the primary PostgreSQL database to verify the system's ability to handle
// the failure and recover gracefully.
//
// This chaos experiment stops the primary database for an extended period while
// read requests are ongoing. It checks that the APIs fail gracefully during the
// outage and then recover once the database is restored.
