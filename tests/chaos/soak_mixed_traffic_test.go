...
func TestChaos_Soak_MixedTraffic_WithInjections(t *testing.T) {
    ...
    _, body, _ = onboard.Request(ctx, "POST", fmt.Sprintf("/v1/organizations/%s/ledgers/%s/accounts", org.ID, ledger.ID), headers, map[string]any{"name":"A","assetCode":"USD","type":"deposit","alias":a})
    _, body, _ = onboard.Request(ctx, "POST", fmt.Sprintf("/v1/organizations/%s/ledgers/%s/accounts", org.ID, ledger.ID), headers, map[string]any{"name":"B","assetCode":"USD","type":"deposit","alias":b})
    ...
    if _, err := h.WaitForAvailableSumByAlias(ctx, trans, org.ID, ledger.ID, a, "USD", headers, decimal.RequireFromString("100.00"), 10*time.Second); err != nil { t.Fatalf("seed wait: %v", err) }
    ...
    for i := 0; i < 3; i++ { wg.Add(1); go traffic() }
