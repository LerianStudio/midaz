...
func TestIntegration_Accounts_GetByAlias(t *testing.T) {
    ...
    if err != nil || code != 200 { t.Fatalf("get by alias: code=%d err=%v body=%s", code, err, string(body)) }
    var got struct{ ID string `json:"id"`; Alias *string `json:"alias"` }
    _ = json.Unmarshal(body, &got)
    if got.ID != acct.ID { t.Fatalf("alias lookup returned different account: want=%s got=%s", acct.ID, got.ID) }
}
