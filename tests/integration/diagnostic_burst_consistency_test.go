...
func TestDiagnostic_BurstConsistency(t *testing.T) {
    ...
    outSucc, inSucc := 0, 0
    mu := sync.Mutex{}
    for i := 0; i < 10; i++ { wg.Add(1); go func(){ defer wg.Done(); c,_,_ := outflow("5.00"); if c == 201 { mu.Lock(); outSucc++; mu.Unlock() } }() }
    for i := 0; i < 20; i++ { wg.Add(1); go func(){ defer wg.Done(); c,_,_ := inflow("2.00"); if c == 201 { mu.Lock(); inSucc++; mu.Unlock() } }() }
    wg.Wait()
