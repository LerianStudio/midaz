...
func TestProperty_Monotonic_CountMetrics(t *testing.T) {
	...
	getCount := func(path string) (int, error) {
		...
		if e != nil || c != 204 {
			return -1, fmt.Errorf("head %s: code=%d err=%v", path, c, e)
		}
	...
	if err != nil {
		t.Fatalf("ledger head initial: %v", err)
	}
	...
	for i := 0; i < 3; i++ {
		...
		if e != nil || c != 201 {
			t.Fatalf("create ledger %d: %d %s", i, c, string(b))
		}
		...
		for {
			...
			if err == nil {
				if cur < last {
					t.Fatalf("ledger count decreased: last=%d cur=%d", last, cur)
				}
				if cur >= last+1 {
					last = cur
					break
			...
			if time.Now().After(deadline) {
				t.Fatalf("ledger count did not increase in time; last=%d", last)
			}
	...
	if e != nil || c != 201 {
		t.Fatalf("create ledger for accounts: %d %s", c, string(b))
	}
	var ledger struct {
		ID string `json:"id"`
	}
	...
	if err != nil {
		t.Fatalf("accounts head initial: %v", err)
	}
	...
	for i := 0; i < 4; i++ {
		...
		if e != nil || c != 201 {
			t.Fatalf("create account %d: %d %s", i, c, string(b))
		}
		...
		for {
			...
			if err == nil {
				if cur < lastAcc {
					t.Fatalf("accounts count decreased: last=%d cur=%d", lastAcc, cur)
				}
				if cur >= lastAcc+1 {
					lastAcc = cur
					break
			...
			if time.Now().After(deadline) {
				t.Fatalf("accounts count did not increase in time; last=%d", lastAcc)
			}
			time.Sleep(100 * time.Millisecond)
		}
	}
