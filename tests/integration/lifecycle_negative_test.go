...
func TestIntegration_Lifecycle_CommitNonPending_Should400(t *testing.T) {
    ...
    // attempting to commit approved/non-pending should be 400
    ...
    if code != 400 { t.Fatalf("expected 400 committing non-pending, got %d body=%s", code, string(body)) }
}
...
func TestIntegration_Lifecycle_RevertNonApproved_Should400(t *testing.T) {
    ...
    // revert should be 400 (non-approved)
    ...
    if code != 400 { t.Fatalf("expected 400 reverting non-approved, got %d body=%s", code, string(body)) }
}
