...
func TestProperty_Pagination_UnionStableNoDuplicates(t *testing.T) {
	...
	fetchPage := func(page int) []string {
		...
		if e != nil || c != 200 {
			t.Fatalf("list page %d: %d %s", page, c, string(b))
		}
		var list struct {
			Items []struct {
	...
	for a, cnt := range seen {
		if cnt > 1 {
			t.Fatalf("duplicate alias across pages: %s", a)
		}
	}
	// Ensure all created aliases are present
	for _, want := range aliases {
		if seen[want] == 0 {
			t.Fatalf("missing alias %s in paginated union", want)
		}
	}
	...
	for i := range p1 {
		if p1[i] != p1b[i] {
			t.Fatalf("unstable order on page 1: %v vs %v", p1, p1b)
		}
	}
