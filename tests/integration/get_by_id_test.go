...
func TestIntegration_GetByID_OrganizationLedgerAccount(t *testing.T) {
	...
	if orgGet.ID != org.ID {
		t.Fatalf("org id mismatch: want %s got %s", org.ID, orgGet.ID)
	}
	...
	if err != nil || code != 200 {
		t.Fatalf("get ledger by id: code=%d err=%v body=%s", code, err, string(body))
	}
	var ledgerGet struct {
		ID string `json:"id"`
	}
	_ = json.Unmarshal(body, &ledgerGet)
	if ledgerGet.ID != ledger.ID {
		t.Fatalf("ledger id mismatch: want %s got %s", ledger.ID, ledgerGet.ID)
	}
	...
	if err != nil || code != 200 {
		t.Fatalf("get account by id: code=%d err=%v body=%s", code, err, string(body))
	}
	var accountGet struct {
		ID string `json:"id"`
	}
	_ = json.Unmarshal(body, &accountGet)
	if accountGet.ID != account.ID {
		t.Fatalf("account id mismatch: want %s got %s", account.ID, accountGet.ID)
	}
}
