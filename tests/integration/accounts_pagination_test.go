...
func TestIntegration_Accounts_ListPagination(t *testing.T) {
	...
	if err != nil || code != 200 {
		t.Fatalf("accounts page1: code=%d err=%v body=%s", code, err, string(body))
	}
	var p1 struct {
		Items []struct {
			ID string `json:"id"`
	...
	if len(p1.Items) == 0 || len(p1.Items) > 2 {
		t.Fatalf("expected 1..2 items on page1, got %d", len(p1.Items))
	}
	...
	if err != nil || code != 200 {
		t.Fatalf("accounts page2: code=%d err=%v body=%s", code, err, string(body))
	}
	var p2 struct {
		Items []struct {
			ID string `json:"id"`
	...
	for _, it := range p2.Items {
		if seen[it.ID] {
			t.Fatalf("duplicate account id across pages: %s", it.ID)
		}
	}
}
