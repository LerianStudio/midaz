...
func TestIntegration_AccountAliasUniqueness_Conflict(t *testing.T) {
    ...
    if cdbg != 200 { t.Fatalf("assets listing status=%d body=%s", cdbg, string(bdbg)) }
    var assets struct{ Items []struct{ Code string `json:"code"` } `json:"items"` }
    _ = json.Unmarshal(bdbg, &assets)
    hasUSD := false
    for _, it := range assets.Items { if it.Code == "USD" { hasUSD = true; break } }
    if !hasUSD { t.Fatalf("USD asset not visible before account creation; assets=%s", string(bdbg)) }
    if err := h.CreateUSDAsset(ctx, onboard, org.ID, ledger.ID, headers); err != nil { t.Fatalf("create USD asset: %v", err) }
    if err := h.CreateUSDAsset(ctx, onboard, org.ID, ledger.ID, headers); err != nil { t.Fatalf("create USD asset: %v", err) }
...
func TestIntegration_AccountsHeadCount(t *testing.T) {
    ...
    for i := 0; i < 2; i++ {
        ...
        if e != nil || c != 201 { t.Fatalf("create account %d: code=%d err=%v body=%s", i, c, e, string(b)) }
    }
    ...
    if after < before+2 { t.Fatalf("accounts head expected increase by >=2, before=%d after=%d", before, after) }
}
