...
func TestIntegration_ReadReplicaAwareness_ReadsSucceedWithReplica(t *testing.T) {
    ...
    // Enable default balance and seed funds so reads have substance
    ...
    if err != nil || code != 200 { t.Fatalf("list ledgers: code=%d err=%v body=%s", code, err, string(body)) }
    ...
    if err != nil || code != 200 { t.Fatalf("list accounts: code=%d err=%v body=%s", code, err, string(body)) }
    ...
    if err != nil || code != 200 { t.Fatalf("get balances by alias: code=%d err=%v body=%s", code, err, string(body)) }
    ...
    for i := 0; i < 20; i++ {
        ...
        code, body, err = onboard.Request(ctx, "GET", fmt.Sprintf("/v1/organizations/%s/ledgers", org.ID), headers, nil)
        if err != nil || code != 200 { t.Fatalf("iteration %d: list ledgers: code=%d err=%v body=%s", i, code, err, string(body)) }
