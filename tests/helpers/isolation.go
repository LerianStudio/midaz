...
// TestIsolation provides methods to generate unique identifiers for test data,
// ensuring that parallel test runs do not interfere with each other.
type TestIsolation struct {
	testRunID string
	timestamp string
}

// NewTestIsolation creates a new test isolation helper, generating a unique ID
...
// UniqueOrgName generates a unique organization name, prefixed for easy identification.
func (ti *TestIsolation) UniqueOrgName(prefix string) string {
	return fmt.Sprintf("%s-%s-%s", prefix, ti.testRunID, RandString(6))
}

// UniqueLedgerName generates a unique ledger name, prefixed for easy identification.
func (ti *TestIsolation) UniqueLedgerName(prefix string) string {
	return fmt.Sprintf("%s-%s-%s", prefix, ti.testRunID, RandString(6))
}

// UniqueAccountAlias generates a unique account alias, prefixed for easy identification.
func (ti *TestIsolation) UniqueAccountAlias(prefix string) string {
	return fmt.Sprintf("%s-%s-%s", prefix, ti.testRunID, RandString(8))
}

// UniqueAssetCode generates a unique, shortened asset code suitable for use in tests.
...
// UniqueTransactionCode generates a unique transaction code, prefixed for easy identification.
...
// TestRunID returns the unique identifier for the current test run.
func (ti *TestIsolation) TestRunID() string {
	return ti.testRunID
}

// MakeTestHeaders creates a set of HTTP headers that includes a unique request ID
// for the current test run.
