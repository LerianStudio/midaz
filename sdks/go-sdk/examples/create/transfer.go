// Package main provides examples of creating resources using the Midaz Go SDK.
package main

import (
	"context"
	"fmt"

	midaz "github.com/LerianStudio/midaz/sdks/go-sdk"
	"github.com/LerianStudio/midaz/sdks/go-sdk/models"
)

// MakeTransfer demonstrates how to create a transfer transaction using the Builder interface.
//
// This function creates a new transfer transaction between two accounts using the builder pattern.
// It shows how to set required fields like organization ID, ledger ID, source and destination
// account references, amount, scale, and asset code, as well as optional fields like description.
//
// Parameters:
//   - ctx: The context for the API request
//   - client: The Midaz client instance
//   - organizationID: The ID of the organization
//   - ledgerID: The ID of the ledger
//   - sourceAccountRef: The reference of the source account
//   - destAccountRef: The reference of the destination account
//   - amount: The amount to transfer (in the smallest unit of the asset)
//   - scale: The scale of the amount (e.g., 2 for cents)
//   - assetCode: The code of the asset (e.g., "USD", "EUR")
//   - description: An optional description for the transaction
//
// Returns:
//   - *models.Transaction: The created transaction
//   - error: An error if the operation fails
func MakeTransfer(
	ctx context.Context,
	client *midaz.Client,
	organizationID, ledgerID string,
	sourceAccountRef, destAccountRef string,
	amount, scale int64,
	assetCode, description string,
) (*models.Transaction, error) {
	// Create a new transfer transaction using the Builder interface
	transferBuilder := client.Builder.NewTransfer().
		WithOrganization(organizationID).
		WithLedger(ledgerID).
		WithSourceAccountReference(sourceAccountRef).
		WithDestinationAccountReference(destAccountRef).
		WithAmount(amount).
		WithScale(scale).
		WithAssetCode(assetCode).
		WithMetadata(map[string]any{
			"type":        "internal_transfer",
			"description": description,
		}).
		WithTags([]string{"transfer", "example"})

	// Create the transfer transaction
	transaction, err := transferBuilder.Create(ctx)
	if err != nil {
		return nil, fmt.Errorf("failed to create transfer: %w", err)
	}

	return transaction, nil
}
