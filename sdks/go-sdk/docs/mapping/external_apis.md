...
- `DepositService.Create()` - Creates a deposit transaction, adding funds to an internal account from an external source
- `DepositService.List()` - Lists deposit transactions with optional filtering
- `DepositService.Get()` - Retrieves a specific deposit transaction by ID
- `DepositService.Update()` - Updates a deposit transaction (e.g., metadata or status)

### Withdrawal Service

- `WithdrawalService.Create()` - Creates a withdrawal transaction, removing funds from an internal account to an external destination
- `WithdrawalService.List()` - Lists withdrawal transactions with optional filtering
- `WithdrawalService.Get()` - Retrieves a specific withdrawal transaction by ID
- `WithdrawalService.Update()` - Updates a withdrawal transaction (e.g., metadata or status)

### Transfer Service

- `TransferService.Create()` - Creates a transfer transaction between two internal accounts
- `TransferService.List()` - Lists transfer transactions with optional filtering
- `TransferService.Get()` - Retrieves a specific transfer transaction by ID
- `TransferService.Update()` - Updates a transfer transaction (e.g., metadata or status)
