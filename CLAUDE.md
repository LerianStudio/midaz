...
- Cross-ledger enablement is a `/v2` policy seam: `settings.crossLedger.enabled` defaults to false and `query.GetCrossLedgerPolicy` resolves each `{organization, ledger}` pair before a future cross-ledger flow consumes it; `/v1` remains unaffected.
