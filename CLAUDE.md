...
Producer is `github.com/LerianStudio/lib-streaming`. Wire format: CloudEvents 1.0 binary mode on Kafka. Topic: `lerian.streaming.<service>_<resource>.<event>` (service = producing product: `ledger` / `fee` / `crm` from the ledger binary, or `tracer` from the tracer service; the route key's hyphens become underscores in the topic name). ce-type is auto-prefixed by lib-streaming as `studio.lerian.<resource>.<event>`. The DefinitionKey / ResourceType / EventType and ce-type are the underscored canonical form (e.g. `operation_route`, `related_party_deleted`); only `RouteDefinition.Key` folds to the hyphenated routing handle (`Definition.RouteKey()`), and the Kafka topic derives from that hyphenated route key. The canonical wire contract lives in code under `pkg/streaming/events/`; the JSONShape unit test in that package locks it against drift.
