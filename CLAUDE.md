...
- Cross-ledger enablement is a `/v2` policy seam: `settings.crossLedger.enabled` defaults to false and `query.GetCrossLedgerPolicy` resolves every participating `{organization, ledger}` pair. `/v2/transactions/direct` decomposes mixed-ledger legs into one balanced transaction per ledger, uses scoped `@external/<asset>` bridge postings, executes all parts atomically, and persists a shared `groupId`. A v2 revert addressed to any member reverses the complete group in one batch, returns a new `groupId` plus `revertedGroupId`, and preserves one parent link per part; `/v1` rejects group members with `0252`. Cross-ledger hold, block, unblock, commit, and cancel remain unsupported. See `docs/api/cross-ledger-transactions.md`.
