...
Producer is `github.com/LerianStudio/lib-streaming`. Wire format: CloudEvents 1.0 binary mode on Kafka. Topic: `{sanitize(service)}.{resource}.{event}` — the sanitized producing-service name is its own leading segment, followed verbatim by the canonical event key. `service` is `ledger` from the ledger binary (which encompasses ledger, fee, and crm events — there is no per-product service segment; fees keep a `fee_` ResourceType prefix, namespacing them inside `ledger.`) or `tracer` from the tracer service. `pkgStreaming.TopicName(service, key)` = `sanitizeServiceSegment(service) + "." + Definition.Key()`: no envelope prefix, no `<service>-` strip, no hyphen fold. ce-type is auto-prefixed by lib-streaming as `studio.lerian.<resource>.<event>`. `Definition.Key()` / ResourceType / EventType / ce-type / the topic are all underscore-canonical (e.g. `operation_route`, `fee_packages`, `related_party_deleted`); only `RouteDefinition.Key` (via `RouteKey()`) is the hyphenated routing handle the lib's route-key validator accepts. Convergence invariant (locked in test): for every cataloged domain event `TopicName(service, def.Key()) == EventDefinition.Topic(ceSource)` when `ceSource` is the bare service name — which is why ce-source is the bare service name (`ledger`/`tracer`), so the emitted topic matches the `{service}.` PREFIXED Kafka ACL byte-for-byte. The billing `active_account` event is the exception: it publishes to the fixed lib topic `lerian.streaming.billing.recorded` as Confluent-framed Protobuf, outside `pkgStreaming.TopicName` and the JSON event catalog. The ledger and tracer binaries each serve `GET /v1/streaming/manifest` (catalog-only). The canonical wire contract lives in code under `pkg/streaming/events/`; the JSONShape unit test in that package locks it against drift.
