...
The reservation lifecycle is a `/v2` contract, gated by the SAME `command.RouteVersionPolicy` the fee seam reads (`RouteV1`/`RouteV2`, `command/transaction_route_version.go`) — threaded from the transport shell because the use cases are transport-agnostic and cannot read the request path. Three seams, all in `components/ledger/internal/services/command/transaction_reservation_anchor.go`, each gating `RouteV1` as their FIRST statement so a `/v1` request returns before a reserve request is built or a connection dialled:
