...
- `reserveTransaction` — create (all six modes) and revert. Called immediately before `ProcessBalanceOperations` on FEE-INCLUSIVE amounts. Only the `/v2` pipelines name it, so a `/v1` request builds no reserve request and dials nothing. Unlike `applyFees` it IS called on a revert: limits measure GROSS activity, so a `/v2` revert reserves capacity of its own.
- `ConfirmReservationsByTransaction` / `ReleaseReservationsByTransaction` — commit / cancel, addressed by transaction id because the create-pending handle does not survive the separate request. These two still take a `command.RouteVersionPolicy` (`RouteV1`/`RouteV2`, `command/transaction_route_version.go`) threaded from the transport shell, and gate `RouteV1` as their FIRST statement — commit/cancel have not been split by version yet.
